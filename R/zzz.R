.onLoad <- function(libname, pkgname) {
  ...
  Ons <- parent.env(environment()) # our own namespace
  if((Rv <- getRversion()) < "4.4.0") {
    assign('%||%', envir = Ons, inherits = FALSE,
    ...
    if(Rv < "4.1.0") {
      ...
      if(Rv < "4.0.0") {
        ...
        if (Rv < "3.2.1") {
        assign('lengths', envir = Ons,
               function (x, use.names = TRUE) vapply(x, length, 1L, USE.NAMES = use.names)
               )
        if(Rv < "3.1.0") {
          assign('anyNA', envir = Ons, function(x) any(is.na(x)) )
          if(Rv < "3.0.0") {
            assign('rep_len', envir = Ons, function(x, length.out) rep(x, length.out=length.out) )
            if(Rv < "2.15") {
              assign('paste0', envir = Ons, function(...) paste(..., sep = '') )
            } ## R < 2.15
          } ## R < 3.0.0
        } ## R < 3.1.0
        } ## R < 3.2.1
