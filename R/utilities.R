...
##'  \item{specials}{a vector indicating which term was generated by a \code{reGenerator} object}
##'  \item{nlambda}{a vector giving the number of non-zero entries in Lambdat for each term}
##'  \item{ntheta}{a vector giving the number covariance parameters for each term}    
...
mkReTrms <- function(bars, fr, reGenerators = NULL) {
	...
	if (length(nl) > 1 && any(diff(nl) != 0)) {
		...
		nl <- nl[ord]
		special <- special[ord]
		# also reorder reGenerators
		if(!is.null(reGenerators)){
			reGenerators <- reGenerators[order(ord[special])]
		}
	}
	
	Zt <- do.call(rBind, lapply(reTrms, "[[", "Zt"))
	q <- nrow(Zt)
	
	## FIXME: do something sensible with the specials here
