...
test.boundary <- function(rho,devfun,b.eps=1e-5) {
    ##
    ## compute finite-difference derivatives for parameters that
    ##  seem to be on the boundary
    ##
    ## FIXME: also test for parameters that are *near* the boundary
    ##   and possibly should be *on* the boundary?
    ##
    ...
    bvals <- which(rho$pp$theta==rho$lower[seq_along(rho$pp$theta)])
    if (length(bvals)>0) {
        ...
        bgrad <- sapply(bvals,
            ...
            bndval <- rho$lower[i]
            theta <- theta0
            theta[i] <- bndval+b.eps
            (devfun(theta)-d0)/b.eps
        })
...
optimizeLmer <- function(devfun,
    ...
    if (restart_edge) {
        bgrad <- test.boundary(rho,devfun)
