...
(fm1a <- lmer2(Reaction ~ Days + (Days|Subject), sleepstudy, REML = FALSE))
(fm2 <- lmer2(Reaction ~ Days + (1|Subject) + (0+Days|Subject), sleepstudy))
anova(fm1, fm2)

## Now works for glmer
fm1. <- glmer2(Reaction ~ Days + (Days|Subject), sleepstudy)
...
(fm3 <- lmer2(Yield ~ 1|Batch, Dyestuff2))
...
(fmX1 <- lmer2(Reaction ~ Days + (Days|Subject), sleepstudy))
(fm.1 <- lmer2(Reaction ~ Days + (1|Subject) + (0+Days|Subject), sleepstudy))
...
fmX1s <- lmer2(Reaction ~ Days + (Days|Subject), sleepstudy, sparseX=TRUE)
...
fm2l <- lmer2(log(Reaction) ~ log(Days+1) + (log(Days+1)|Subject),
...
try(reg <- lmer2(y ~ habitat + (1|habitat*lagoon), data = dat) # did seg.fault
    ) # now gives error                 ^- should be ":"
r1  <- lmer2(y ~ 0+habitat + (1|habitat:lagoon), data = dat) # ok, but senseless
r1b <- lmer2(y ~ 0+habitat + (1|habitat), data = dat) # same model, clearly indeterminable
...
r2.0 <- lmer2(y ~ 0+lagoon + (1|habitat:lagoon), data = dat) # ok
r2   <- lmer2(y ~ 0+lagoon + (1|habitat), data = dat) # ok, and more clear
...
r2.  <- lmer2(y ~ 0+lagoon + (1|habitat), data = dat,
...
if (FALSE) {  # mcmcsamp still needs work
    ...
    y <- 1:10
    group <- gl(2,5)
    (M1 <- lmer2 (y ~ 1 + (1 | group))) # works fine
    (r1 <- mcmcsamp (M1))              # dito
    ...
    M2 <- lmer2 (y ~ 1 + x + (1 + x | group)) # false convergence
    ## should be identical (and is)
    M2 <- lmer2 (y ~ x + ( x | group))#  false convergence -> simulation doesn't work:
...
## --> do need CPU
m0 <- lmer2(y ~ (x1 + x2)|ff, data = D)
m1 <- lmer2(y ~ x1 + x2|ff  , data = D)
m2 <- lmer2(y ~ x1 + (x2|ff), data = D)
m3 <- lmer2(y ~ (x2|ff) + x1, data = D)
stopifnot(all.equal(ranef(m0), ranef(m1)),
          all.equal(ranef(m2), ranef(m3)),
          inherits(tryCatch(lmer2(y ~ x2|ff + x1, data = D), error = function(e)e),
                   "error"))
