...
m1 <- glmer2(cbind(incidence, size - incidence) ~ period + (1 | herd),
             family = binomial, data = cbpp)
## response as a vector of probabilities and usage of argument "weights"
m1p <- glmer2(incidence / size ~ period + (1 | herd), weights = size,
             family = binomial, data = cbpp)
...
m1b <- glmer2(cbind(incidence, size - incidence) ~ period + (1 | herd),
             optimizer="bobyqa",
...
m.9 <- glmer2(cbind(incidence, size - incidence) ~ period + (1 | herd),
             family = binomial, data = cbpp, nAGQ = 9)

## check with nAGQ = 25
m2 <- glmer2(cbind(incidence, size - incidence) ~ period + (1 | herd),
...
if (require('MASS', quietly = TRUE)) {
    ...
    print(fm5 <- glmer2(y ~ trt + wk2 + (1|ID),
...
tc <- tryCatch(
	       m2 <- glmer2(incidence / size ~ period, weights = size,
			   family = binomial, data = cbpp)
	       , error = function(.) .)
...
set.seed(1)
dd <- rPoisGLMMi(12, 20)
m0  <- glmer2(y~x + (1|f),           family="poisson", data=dd)
m1 <- glmer2(y~x + (1|f) + (1|obs), family="poisson", data=dd)
