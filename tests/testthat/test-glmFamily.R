...
## Regression tests for issue #936: sigma estimation for GLMMs with scale
## parameters was biased because the C++ aic() profiled sigma from the
## conditional deviance alone, ignoring that ldL2 depends on sigma through
## the working weights.  Reference values from glmmTMB were generated by
## inst/testdata/test-scale-glmm_data.R and stored in the .rds below.
scale_glmm_ref <- readRDS(
  system.file("testdata", "test-scale-glmm_data.rds", package = "lme4")
)

test_that("gaussian(link='log') sigma matches glmmTMB (issue #936)", {
  data("Rail", package = "nlme")
  m_lme4 <- suppressWarnings(
    glmer(travel ~ 1 + (1|Rail), data = Rail, family = gaussian(link = "log"))
  )
  ## sigma and fixef should agree with glmmTMB reference to within ~0.5%
  expect_equal(sigma(m_lme4),          scale_glmm_ref$gauss_sigma, tolerance = 0.005)
  expect_equal(unname(fixef(m_lme4)),  scale_glmm_ref$gauss_fixef, tolerance = 0.005)
  ## regression value
  expect_equal(sigma(m_lme4), 4.0082319270113, tolerance = 1e-6)
})

test_that("Gamma sigma matches glmmTMB", {
  m_lme4 <- glmer(y ~ 1 + (1|group), family = Gamma(link = "log"), data = dd2)
  ## sigma should agree with glmmTMB reference to within ~0.5%
  expect_equal(sigma(m_lme4), scale_glmm_ref$gamma_sigma, tolerance = 0.005)
})
