...
#' @title Variance heterogeneous correlated random effects (compound symmetry)
#' 
...
#' 		  If not supplied, sd's will be 1 and the correlation will be 0.3.	    
#' @return a function creating a return object like \code{mkReTrm}    
#TODO: more thinking to be done here --
#  allowing negative correlations messes yields 
#  non - +ive definiteness... 
vh <- function(formula, init=NULL){
	mkReTrmVh <- local({
		bar <- formula[[2]][[2]]
		function(fr){
			ff <- getGrouping(bar, fr)
