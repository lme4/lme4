...
* the scaled-gradient points are largely invisible because in this case, the scaled gradient is almost always smaller than the absolute gradient, so the `maxmingrad` point (which is drawn later) covers the scaled-gradient point except in a few cases.
...
* I use log10 of the gradients throughout below, even where I forget to label them as such (perhaps obvious because they're negative ...)
...
gg_bad <-
    ...
    scale_colour_brewer(palette="Dark2")+
        geom_smooth(method="gam",aes(weight=n),se=FALSE,
...
(gg_bad3 <- ggplot(bad_ests3,aes(x=grad,y=bad,colour=optimizer))+
...
If we turn off Hessian computations for large data we don't have
access to the scaled gradient; then we could set the gradient criterion
to 0.5 (for this *particular* example; presumably it would be a bit different
for other cases ...)

```{r}
red3 <- m %>% filter(var=="maxgrad") %>%
    select(-c(data,var)) %>%
        rename(grad=val)
bad_ests4 <- full_join(bad_ests2,red3) %>% ungroup() %>%
    mutate(sfac=factor(floor(log10size)))
gg_bad3 %+% bad_ests4+ geom_vline(xintercept=log(0.5),lty=2)
```
