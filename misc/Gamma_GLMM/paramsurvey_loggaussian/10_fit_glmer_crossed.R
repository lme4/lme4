...
for (case in names(sim$cases)) {
  ...
  fitfun <- switch(case,
                    randomslope10   = fit_glmer_randomslope_one,
                    nested5         = fit_glmer_nested_one,
                    crossedslopes5  = fit_glmer_crossedslopes_one,
                    if (grepl("^oneway", case)) fit_glmer_oneway_one else fit_glmer_crossed_one)
  ...
  outfile <- file.path(wd, sprintf("crossed_results_glmer_%s_%s.rds", case, family_name))
  saveRDS(df, outfile)
  cat("\nsaved to", outfile, "\n\n")
}
