...
cat("Fitting MASS::glmmPQL to", sim$B, "replicates of", sim$name, "\n")

results <- lapply(seq_len(sim$B), function(b) fit_pql_one(b, sim$sim_data[[b]]))
...
saveRDS(df, file.path(wd, "rail_results_pql.rds"))
cat("\nsaved to", file.path(wd, "rail_results_pql.rds"), "\n")
