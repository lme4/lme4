...
cat("Fitting PIRLS/", phiType, "-phi to ", sim$B, " replicates of ", sim$name,
    " -- mc.cores = ", MC_CORES, "\n", sep = "")

applyfun <- if (MC_CORES > 1) function(...) mclapply(..., mc.cores = MC_CORES) else lapply
results <- applyfun(seq_len(sim$B), function(b) fit_pirls_phi_one(b, spec, sim$sim_data[[b]], phiType = phiType))
