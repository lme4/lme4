...
## fits, one plot per method (lme4 2.0-6, lme4 current, lme4 joint-phi,
## glmmTMB). The joint-phi arm shares lme4's native raw-Cholesky theta
## parameterization (built via the same glFormula machinery), so it's
## directly eyeball-comparable to the other two lme4 panels, unlike glmmTMB.
...
df_cur <- makeDF(r_cur)
df_tmb <- makeDF(r_tmb)
df_joint <- makeDF(r_joint)

cat("n complete cases: old =", nrow(df_old), " current =", nrow(df_cur),
    " joint-phi =", nrow(df_joint), " glmmTMB =", nrow(df_tmb), "\n")
...
png(file.path(outdir, "multistart_pairs_jointphi.png"), width = 1400, height = 1400, res = 130)
pairs(df_joint, gap = 0, main = sprintf("lme4 joint-phi: theta (native), beta, sigma (n=%d)", nrow(df_joint)))
dev.off()

cat("saved multistart_pairs_lme4old.png, multistart_pairs_lme4current.png, multistart_pairs_glmmTMB.png, multistart_pairs_jointphi.png\n")
