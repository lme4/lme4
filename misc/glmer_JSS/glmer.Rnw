...
\fixme{Discuss two-stage fitting (nAGQ=0, then nAGQ $>0$) ?}
...
bootVars <- load("glmer_bootbatch.rda")
profVars <- load("glmer_profbatch.rda")
@
...
combCI0 <- do.call(rbind, c(
  mapply(mfun, list(confint0.boot, confint0.wald),
         c("boot","Wald"), "gm1", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof, confint.boot, confint.wald),
         c("profile","boot","Wald"), "gm2", SIMPLIFY=FALSE),
  mapply(mfun, list(confint3.prof, confint3.boot, confint3.wald),
