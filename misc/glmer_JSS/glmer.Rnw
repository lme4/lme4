...
load("CBPP_batch.rda")
CBPP_batch_vars <- load("CBPP_profbatch.rda")
@
...
mfun <- function(x, type, model) {
   ...
   value_cols <- setdiff(names(df), "var")
   m <- reshape(
          df,
          varying = value_cols,
          v.names = "value",
...
my_dcast <- function(x) {
  x_wide <- reshape(x,
     idvar = c("var", "type", "model"),
     timevar = "variable",
     direction = "wide"
   )
   names(x_wide) <- sub("^value\\.", "", names(x_wide))
   x_wide
}

combCI <- merge(my_dcast(combCI0), combEst2, 
