...
\fixme{Figure 8: why are some profile results still wonky?}
...
\fixme{Figure 10: why are age estimates different for additive models even though I've set sum-to-zero contrasts ? Where did cor parameter go for the one model that has it?}
