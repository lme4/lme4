...
linear models (GLMs). In a companion paper \cite{lmerJSS}, we have described the
...
\al{
Assuming that the conditional distribution is in the exponential 
dispersion family (e.g. binomial, Poisson, Gamma)
%% BMB: neg binom is not in the exponentional dispersion unless the
%% shape parameter is known; inverse-Gaussian is also ED)
...
If prior weights are not specified, by default $\bm w$ is just a vector of ones.

Another common feature of generalized linear mixed models is the use
of an \textbf{offset}, a practical way to include known scaling 
factors (e.g., population or area over which a Poisson sample is collected)
without introducing additional parameters to the model.

In particular, the inclusion of an offset modifies the linear predictor 
as follows:
\begin{equation}
  \label{eq:GLMMoffset}
...
\al{
For GLMMs, the scale parameter is estimated outside of the optimization 
of the likelihood over the $\beta$ and the $\theta$ parameters. It is estimated 
by the deviance divided by the residual degrees of freedom.
...
described in Chapter 5.1 of the \code{lmer} paper \citep{bates2015fitting}.}
