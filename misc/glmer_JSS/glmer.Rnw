...
\fixme{what's causing the empty first page?}

\fixme{Discuss nAGQ=0, and two-stage fitting (nAGQ=0, then nAGQ $>0$) ?}

\al{I added a few paragraphs to talk about nAGQ=1 and nAGQ>1, but unsure where to
put nAGQ=0..}

\fixme{
  (1) See \url{https://kaskr.github.io/adcomp/_book/Appendix.html} for
  details of when nAGQ=0 is appropriate; (2) ask Doug Bates and/or
  Kasper K. where the idea comes from? (3) discuss when nAGQ=0
  is likely to work [conditional mean = conditional mode when
  effective sample size per group is large, e.g. CLT - similar to
  conditions for adequacy of Laplace approx.;
  linearity of $\partial(\log{\cal L})/\partial \beta$ with respect
  to $u$ works for LMMs, when else? When link function is not too nonlinear?]}
...
For GLMMs, the scale parameter is not estimated as part of the nonlinear minimization of the negative likelihood over the $\beta$ and $\theta$ parameters.
...
residual sum of squares, as described in Section 3.2 of the \code{lmer} paper \citep{bates2015fitting}.
...
<<confPlot,fig.cap="CBPP example: comparison of point and confidence interval estimation for different methods.",fig.scap="CBPP comparison",echo=FALSE,fig.width=7,fig.height=5>>=
...
compound symmetry, and first-order autoregressive (AR1) structures. Both 
homogeneous and heterogeneous forms are available for diagonal, compound 
symmetric, and AR1 structures; by default, AR1 is assumed homogeneous, while 
the others are heterogeneous.
...
To fit a heterogeneous structure, use \code{ar1(..., hom = FALSE)}.
For this particular model, a heterogeneous AR1 model results in a singular fit.
...
mnamevec <- c("int_child    + age  + (1 | district)",
              "binary_child + age  + (1 | district)",
                ...
                "binary_child × age + (1 | district:urban")  
AICtab(cm1,cm2,cm3,cm4,cm5,cm6,
                mnames = mnamevec)
options(op)  
@

\al{Between these six models, the AIC, BIC, and negative log likelihood 
are all similar to one another. The top model by AIC is \code{binary\_child} $\times$ \code{age + (1 \textbar{} district:urban)}, \code{(binary\_child} $\times$ \code{age + (1 \textbar{} district:urban))}, though model \code{(binary\_child} $\times$ \code{age + (1 \textbar{} district/urban))} has an equivalent negative log likelihood score but higher AIC ($\Delta$AIC = 2.0, $\Delta$negloglik = 0.00).

The model with a random slope for urban across districts (model \code{binary\_child} $\times$ \code{age + (urban \textbar{} district)}) fits the data well in terms of log likelihood ($\Delta$negloglik = 0.00) but is penalized for its extra parameter ($\Delta$AIC = 3.1).

Models that drop the interaction between child status and age or replace the binary child indicator with an integer count perform substantially worse, with $\Delta$AIC values of 10.7, 16.8, and 20.3 respectively.

Overall, the results suggest that accounting for urban/rural variation at the district level and including the age and child interaction are both important, but the precise random effects structure for urban (crossed vs.\ nested vs.\ slope) has relatively little impact on fit.}

Similar to the CBPP data set, another relevant demonstration is computing 
profile, Wald, and bootstrapping confidence intervals for one of the models to
...
ggplot(combCI, aes(var, est, colour=type, shape=model)) +
  ...
  coord_flip() +
  scale_colour_brewer(palette="Dark2") +
  scale_shape(labels = mnamevec, guide = guide_legend(reverse = TRUE)) +
...
In general, the point and confidence interval estimation of the explanatory variables are similar across the six models, and similarly across different methods for confidence interval construction, with a couple of exceptions.
...
We may also impose a different underlying variance-covariance structure. 
Consider the following models, where \code{diag} enforces a diagonal 
variance-covariance structure, and \code{cs} enforces a compound symmetric model:


<<cm_varcorr,eval=FALSE,echo=FALSE>>=
