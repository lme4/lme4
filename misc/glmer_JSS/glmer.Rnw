...
varcorr_vals <- load("glmer_cm_vc.RData")
@
...
We construct six different \code{glmer} models with varying choices of explanatory variables and grouping terms, and compare them using \code{anova}. 

  \fixme{describe the different cases (whether we dichotomize children to ``any living children'', interaction of children with age, random effects structure)}
  
Across the six models, we vary the specification of explanatory variables, 
such as treating child count as a factor (0, 1, 2, 3+) or binary (no living children vs having living children), and explore different grouping structures like district or urban rural.
...
Between these six models, the AIC, BIC, and negative log likelihood 
...
## see the file glmer_contraception_batch.R...
L3 <- load(file="glmer_profbatch3.RData")
L4 <- load(file="glmer_profbatch4.RData")
@

<<ContraceptionConfidenceSetup,echo=FALSE>>=
combCI0 <- do.call(rbind, c(
  mapply(mfun, list(confint.boot.cm1_2, confint.wald.cm1_2),
         c("boot","Wald"), "cm1", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.boot.cm2_2, confint.wald.cm2_2),
         c("boot","Wald"), "cm2", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof.cm3_2, confint.boot.cm3_2, confint.wald.cm3_2),
         c("profile", "boot","Wald"), "cm3", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof.cm4_2, confint.boot.cm4_2, confint.wald.cm4_2),
         c("profile", "boot","Wald"), "cm4", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof.cm5_2, confint.boot.cm5_2, confint.wald.cm5_2),
         c("profile", "boot","Wald"), "cm5", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof.cm6_2, confint.boot.cm6_2, confint.wald.cm6_2),
         c("profile", "boot","Wald"), "cm6", SIMPLIFY=FALSE)
))

combCI0 <- combCI0[combCI0$var %in% c("urbanY", "I(age^2)", "chY", 
                                      "age:chY", "age"), ]

combEst2 <- do.call(rbind, list(
  get_est(cm1, "cm1"), get_est(cm2, "cm2"), get_est(cm3, "cm3"), 
  get_est(cm4, "cm4"), get_est(cm5, "cm5"), get_est(cm6, "cm6")
))
...
\fixme{why does Fig 10 have only fixed effect values? It would help if combCI were renamed instead of reused between cbpp and Contraception examples}

<<confPlot2,fig.cap="Contraception example: comparison of point and confidence interval estimation for different methods.",fig.scap="Contraception comparison",echo=FALSE,fig.width=8,fig.height=10,warning=FALSE>>=
ggplot(combCI, aes(var, est, colour=type, shape=model)) +
  geom_pointrange(aes(ymin=lwr, ymax=upr),
                  position=position_dodge(width=0.6)) +
  labs(x="", y="Estimate (log-odds of seropositivity)") +
