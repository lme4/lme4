...
All three of these confidence intervals are compared against each other
in Figure~\ref{fig:confPlot}. (Should comments be made about the comparison?)
}
...
\al{
Similar to the CBPP data set, we will provide a demonstration of computing 
...
contCI0 <- do.call(rbind,
                   mapply(mfun,list(confint.prof.cm3,confint.boot.cm3,
                                    confint.wald.cm3),
                          c("profile","boot","Wald"),SIMPLIFY=FALSE))
v <- as.data.frame(VarCorr(cm3))
vnames <- paste0("sd_",v$var1,"|",v$grp)
contEst <- c(fixef(cm3),setNames(v$sdcor,vnames))
contEst2 <- data.frame(var=names(contEst),est=contEst,row.names=NULL)
contCI <- merge(dcast(contCI0,var+type~variable),contEst2,all.x=TRUE)
contCI <- transform(contCI,var=factor(var,levels=as.character(contEst2$var)))
contCI <- subset(contCI,var!="(Intercept)")
@

<<confPlot2,fig.cap="Contraception example: comparison of point and confidence interval estimation for different methods",fig.scap="Contraception comparison",echo=FALSE,fig.width=7,fig.height=3.5>>=
ggplot(contCI,aes(var,est,colour=type))+
  geom_pointrange(aes(ymin=lwr,ymax=upr),
                  position=position_dodge(width=0.2))+
  labs(x="",y="Estimate")+
  geom_hline(yintercept=0,lty=2)+
...
vc.hetdiag <- VarCorr(cm.hetdiag)
vc.cs <- VarCorr(cm.cs)
vc.hetcs <- VarCorr(cm.hetcs)
...
\al{
Need to ask the above for more details....
}
