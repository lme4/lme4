...
We will show examples of \code{allFit()\$which.OK} and \code{allFit()\$llik}.
...
Exact details of the implementation can be found in the \code{lme4} covariance structure vignette. For now, we elaborate on how one can extract the variance-covariance matrix for the specific grouping term \code{period} using \code{VarCorr}.
}

<<varcorr_plot_gm1.ar1,fig.cap="CBPP example: variance-covariance matrix of the grouping term period.",fig.scap="CBPP VarCorr",echo=FALSE,fig.width=7>>=
...
\al{We construct six different `glmer` models with varying choices of explanatory variables and grouping terms, and compare them using `anova`.}
...
print(summary(cm1 <- glmer(use ~ age + I(age^2) + urban + livch + (1|district), 
                           ...
                           control = glmerControl(autoscale=TRUE))), corr = FALSE)
@
...
print(summary(cm5 <- glmer(use ~ age*ch + I(age^2) + urban + (1|urban:district) + (1|district), control = glmerControl(autoscale=TRUE),
...
<<ContraceptionBatch,eval=FALSE,echo=FALSE>>=
load(file="glmer_bootbatch2.RData")
load(file="glmer_profbatch2.RData")
@

<<ContraceptionConfidenceCalc,eval=FALSE,echo=FALSE>>=
...
rownames(confint.boot.cm3)[1] <- rownames(confint.prof.cm3)[1]
@
...
combEst2 <- do.call(rbind, list(
  ...
  get_est(cm4, "cm4"), get_est(cm5, "cm5"), get_est(cm5, "cm5")
))
