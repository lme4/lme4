...
\newcommand{\bbeta}{\ensurement{\bm \beta}}
\newcommand{\btheta}{\ensurement{\bm \btheta}}
...
\fixme{Figure 8: scale avg size, separate sd into separate facet}
...
We will show an example of \code{allFit()\$which.OK} The rest are similar.

<<allfit_cbpp1,message=FALSE,warning=FALSE>>=
gm_all <- allFit(gm1)
@

<<allfit_cbpp2,eval=FALSE>>=
ss <- summary(gm_all)
ss$which.OK 
@
...
contra_variables <- unique(combCI0$var)

exclude <- c("livch1", "livch2", "livch3+")
combCI0 <- combCI0[combCI0$var %in% setdiff(contra_variables, exclude), ]
