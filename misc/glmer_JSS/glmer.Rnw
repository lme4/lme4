...
\Plainauthor{Anna Ly, Steven C. Walker, Rune Haubo Bojesen Christensen, Douglas Bates, Martin M\"achler, Benjamin M. Bolker}
...
\al{
Once we can calculate the deviance by PIRLS for specified values of $\btheta$ and $\bbeta$ (or only $\btheta$ if profiling out the fixed effects via \code{nAGQ=0}), we then estimate the parameters by nonlinear optimization. This procedure largely follows the description in \cite{bates2015fitting}, using derivative-free optimizers with box constraints to prevent non-positive-(semi)definite covariance matrices. Specifically, the elements of $\btheta$ corresponding to the diagonal of $\bm\Lambda_\theta$ are currently constrained to be non-negative. We note, however, that the profiled log-likelihood is an even function of these diagonal parameters: since $\bm\Sigma_\theta=\bm\Lambda_\theta\bm\Lambda_\theta\trans$ depends on them only through their squares, positive and negative values yield identical likelihoods. This symmetry means that constrained optimization is not strictly necessary — an unconstrained optimizer will converge to the correct solution, approaching zero from either side in the boundary case of a singular random-effects covariance matrix. Removing this constraint would permit the use of a broader class of unconstrained optimizers, though this has not yet been incorporated into the implementation. The only difference is that by default \code{glmer} uses a two-step fitting procedure, using \code{nAGQ=0} at the first stage to get preliminary estimates which are then used as starting points for a second optimization with Laplace approximation or Gauss-Hermite quadrature as specified by the user. Different nonlinear optimizers can be used at each stage: the current default, as specified in \code{glmerControl}, is to use Powell's BOBYQA followed by a box-constrained variant of the Nelder-Mead simplex algorithm. For faster, approximate fitting, the second stage can be omitted; in the rare cases where the initial \code{nAGQ=0} fit gives poor results, the first stage can be skipped via \code{glmerControl(nAGQ0initStep = FALSE)}.
}
...
other four include a \code{ch} and \code{age} interaction. \al{The effect of 
age on contraceptive use is not linear, which is why \code{I(age\^{}2)}
is included.}
...
in the data set) was standardized by scaling by two 
standard deviations \citet{gelman2008scaling}.
...
\fixme{Figure~\ref{fig:contracompplot}: why are age estimates different for additive models even though I've set sum-to-zero contrasts ? Where did cor parameter go for the one model that has it?}
...
\al{we have a cs model below; may need to write interpretations...}
<<cm_varcorr>>=
...
cm.cs <- glmer(use ~ age*ch + I(age^2) + urban + cs(1 + urban|district),
              Contraception_scale, binomial)
cm.hetcs <- glmer(use ~ age*ch + I(age^2) + urban + cs(1 + urban|district, hom=FALSE),
                   Contraception_scale, binomial)
@
