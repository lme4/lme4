...
\al{Fitting generalized linear mixed models requires approximating an intractable 
integral over the random effects distribution. The \code{glmer} function offers 
several approximations, controlled by the \code{nAGQ}. 
At the default \code{nAGQ=1}, the Laplace approximation is used.}
...
\fixme{is this left over?}
\al{This was here to begin with... So I'm good with removing it.}
...
\al{To use the adaptive Gauss-Hermite quadrature approach for model fitting 
in \code{glmer} models, ensure that the argument \code{nAGQ}, which is the 
number of knots $k$, is greater than 1.
