...
\newcommand{\al}[1]{{\color{violet}#1}} % anna's new comments
...
\author{
  ...
  Anna Ly\\University of Toronto \And
...
\Plainauthor{Douglas Bates, Anna Ly, Steve Walker, Rune Haubo Bojesen Christensen, Douglas Bates, Martin M\"achler, Benjamin M. Bolker}
...
\fixme{add 'toenail' example for bad Laplace?}
...
$\bm\Lambda_\theta$, which is a matrix square root of $\Sigma_\theta$ (in practice a Cholesky factor),
\begin{equation}
  \label{eq:GLMMSigma}
...
If prior weights are not specified, by default $\bm w$ is just a vector of 1.

Another common implementation when dealing with generalized linear mixed models
is an \textbf{offset}, which are an efficient way to include known scaling 
...
Increasing the number of nodes improves the accuracy of the likelihood 
approximation at the expense of computation time. However, AGQ is only available 
for models with a single scalar random effect.

\fixme{move this?}
The \code{lme4} package supports fitting negative binomial GLMMs,
which are an extension of GLMMs; the negative binomial family is within the
exponential dispersion family only if the dispersion parameter is fixed.
The \code{glmer.nb} function implements a one-dimensional optimization
procedure to estimate the dispersion parameter, fitting a negative binomial GLMM with
a fixed dispersion parameter at each trial value. While correct, this
approach is slower than other available negative binomial GLMM
implementations (e.g., as offered in the \code{glmmTMB} package),
which is recommended for large problems.
...
mm <- transform(mm, percent=prop/100)
mm <- transform(mm,incidence=round(percent*size))
cbpp2 <- na.omit(subset(mm,select=c(herd,treatment,avg_size,
...
<<cbppPlot,echo=FALSE,fig.cap="Incidence (proportion of cows becoming seropositive per observation period) vs. period. Colours show treatment category for each herd; point sizes reflect the number of seronegative cows at the start of each period.",fig.scap="Incidence">>=
...
<<glmerRanefplot,fig.cap="Graphical display of random effects. \\emph{Left}: conditional modes $\\pm 1.96 \\times \\text{conditional mode}$, ordered by magnitude. \\emph{Right}: quantile-quantile plot, with linear regression line overlaid.",echo=FALSE>>=
...
the normality of the distribution of conditional modes.
Figure~\ref{fig:perf_chk_mod_cbpp} shows an example.

<<perf_chk_mod_cbpp,fig.cap="Model diagnostics using \\code{performance::check\\_model}",fig.height=10,echo=FALSE>>=
check_model(gm1, check = c("reqq", "binned_residuals", 
                                        "vif", "qq"))
@
...
<<dharma_mod_cbpp,fig.cap="Model assumption checks using the DHARMa package. \\code{n.s.} denotes ``not significant.''",echo=FALSE>>=
...
cbpp_batch_vars <- load("CBPP_batch.rda")
contr_batch_vars <- load("Contraception_batch.rda")
@
...
profiles, each of which requires many $p-1$-dimensional optimizations.  The machinery for generating likelihood profiles for GLMMs is similar to that for LMMs (see \citep{bates2015fitting}, \S\ 5.1).
...
Two other common methods for comuputing confidence intervals
are the classical Wald approximation (\code{method = "Wald"}) and parametric 
bootstrapping (\code{method = "boot"}).
The Wald approximation is faster and less accurate than profile
confidence intervals (and only returns estimates for the fixed-effects
...
Figure~\ref{fig:confPlot} compares all three of these confidence intervals
across all three of the models fitted.

<<cbppConfidence,echo=FALSE>>=
mfun1 <- function(x, model, model_id) {
...
mfun2 <- function(x, type, est) {
  df <- do.call("rbind", Map(mfun1, x, df_name$mdesc, df_name$mnames))
  df <- merge(df, contr_est, by.x = c("model_id", "var"), by.y = c("model", "var"))
  df$type <- type
  df
}
## mfun1(contr_confint_wald[[1]], "hello", "h")
## do.call("rbind", Map(mfun1, contr_confint_wald, df_name$mdesc, df_name$mnames))
## mfun2(contr_confint_wald, "wald")
combCI <- do.call("rbind", Map(mfun2,
...
\fixme{Figure 8: why are some profile results missing?}

<<confPlot,fig.cap="CBPP example: comparison of point and confidence interval estimation for different methods.",fig.scap="CBPP comparison",echo=FALSE,fig.width=7,fig.height=5,warning=FALSE>>=
mnames <- c("herd","herd+obs","obs")
## save("combCI", "mnames", file = "pix_tmp.rda")
...
<<load_cm_varcorr,echo=FALSE>>=
varcorr_vals <- load("glmer_cm_vc.rda")
@
...
See the \href{https://cran.r-project.org/web/packages/lme4/vignettes/covariance_structures.html}{lme4 covariance structures vignette}
for more detail.
...
<<graphContraception,echo=FALSE,fig.cap="Contraception example: proportion of contraceptive use by centered age, number of living children (line type), and urban/rural residence (facet). Curves fitted using generalized additive models with cubic spline smoothing.">>=
Contraception <- within(Contraception, 
                        facet_urban <- ifelse(urban=="Y","Urban","Rural"))
ggplot(Contraception) + 
    geom_smooth(aes(age, ifelse(use == "Y",1,0), colour = livch),
...
<<contraception_scale, echo=FALSE>>=
# Anna: scaled and centered the variables here to improve interpretability
...
Contraception_scale <- gelman_scale(Contraception, "use")
@
...
other four include a \code{ch} and \code{age} interaction. 

Third, we explore different random effects structures at the district level.
Three of them use a single random intercept per district \code{(1 \textbar{} district)}.
One of them extends this with a random slope for urban status 
\code{(urban \textbar{} district)}, allowing the urban/rural gap to vary by district. 
Another one uses two nested random intercepts \code{(1 \textbar{} district)} and 
\code{(1 \textbar{} urban:district)}, separating district-level from urban-within-district variation. The remaining model uses only the \code{urban:district} grouping. 

To reduce convergence warnings and facilitate interpretability, all 
explanatory variables were standardized by centering and scaling to two 
standard deviations, following the approach recommended by \citet{gelman2008scaling}.

<<cm1,message=FALSE>>=
cm1 <- glmer(use ~ age + I(age^2) + urban + livch + (1|district), 
                           Contraception_scale, binomial, nAGQ=0L)
@

<<cm2,echo=FALSE,message=FALSE,size='scriptsize'>>=
cm2 <- glmer(use ~ age + I(age^2) + urban + ch + (1|district),
                           Contraception_scale, binomial, nAGQ=0L)
@

<<cm3,echo=FALSE,message=FALSE,size='scriptsize'>>=
cm3 <- glmer(use ~ age*ch + I(age^2) + urban + (1|district),
                           Contraception_scale, binomial)
@

<<cm4,echo=FALSE,message=FALSE,size='scriptsize'>>=
cm4 <- glmer(use ~ age*ch + I(age^2) + urban + (1+urban|district),
              Contraception_scale, binomial)
@

<<cm5,echo=FALSE,message=FALSE,size='scriptsize'>>=
cm5 <- glmer(use ~ age*ch + I(age^2) + urban + (1|urban:district) + (1|district),
                           Contraception_scale, binomial)
@

<<cm6,echo=FALSE,message=FALSE,size='scriptsize'>>=
...
mnamevec <- c("int_child    + age  + (1 | district)",
              "binary_child + age  + (1 | district)",
                ...
                "binary_child × age + (1 | district:urban")  
print(AICtab(cm1, cm2, cm3, cm4, cm5, cm6,
                mnames = mnamevec))
options(op)  
@
...
profile, Wald, and parametric bootstrap confidence intervals for one of the models to
understand the effect of each variable.

<<ContraceptionBatch,echo=FALSE>>=
## see the file glmer_contraception_batch.R and
## contraception_rename.R
ContraBoot <- load(file="Contra_boot_sigFALSE.rda")
ContraProf <- load(file="Contra_prof_sigFALSE.rda")
ContraWald <- load(file="Contra_wald_sigFALSE.rda")
@

<<ContraceptionConfidenceSetup,echo=FALSE>>=
contr_combCI0 <- do.call(rbind, c(
  mapply(mfun, list(confint.boot.cm1, confint.wald.cm1),
         c("boot","Wald"), "cm1", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.boot.cm2, confint.wald.cm2),
         c("boot","Wald"), "cm2", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof.cm3, confint.boot.cm3, confint.wald.cm3),
         c("profile", "boot","Wald"), "cm3", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof.cm4, confint.boot.cm4, confint.wald.cm4),
         c("profile", "boot","Wald"), "cm4", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof.cm5, confint.boot.cm5, confint.wald.cm5),
         c("profile", "boot","Wald"), "cm5", SIMPLIFY=FALSE),
  mapply(mfun, list(confint.prof.cm6, confint.boot.cm6, confint.wald.cm6),
         c("profile", "boot","Wald"), "cm6", SIMPLIFY=FALSE)
))

contra_variables <- unique(contr_combCI0$var)

exclude <- c("livch1", "livch2", "livch3+")
contr_combCI0 <- contr_combCI0[contr_combCI0$var %in% setdiff(contra_variables, exclude), ]

get_est2 <- function(mod, model_name) {
  v <- as.data.frame(VarCorr(mod))
  sd_rows <- v[is.na(v$var2), ]
  cor_rows <- v[!is.na(v$var2), ]
  vnames_sd <- paste0("sd_", sd_rows$var1, "|", sd_rows$grp)
  vnames_cor <- paste0("cor_", cor_rows$var2, ".", cor_rows$var1, "|", cor_rows$grp)
  est <- c(fixef(mod), setNames(sd_rows$sdcor, vnames_sd))
  if( length(cor_rows$sdcor) > 0 ){
    est <- c(est, setNames(cor_rows$sdcor, vnames_cor))
  }
  data.frame(model=model_name, var=names(est), est=est, row.names=NULL)
}

combEst2 <- do.call(rbind, list(
  get_est2(cm1, "cm1"), get_est2(cm2, "cm2"), get_est2(cm3, "cm3"), 
  get_est2(cm4, "cm4"), get_est2(cm5, "cm5"), get_est2(cm6, "cm6")
))

contr_combCI <- merge(my_dcast(contr_combCI0), combEst2, 
                by=c("var","model"), all.x=TRUE)

contr_combCI <- transform(contr_combCI, model_type = interaction(model, type, sep="\n"))

mnamevec <- c("int_child    + age  + (1 | district)",
              "binary_child + age  + (1 | district)",
              "binary_child × age + (1 | district)",
              "binary_child × age + (urban | district)",
              "binary_child × age + (1 | district/urban)",
              "binary_child × age + (1 | district:urban")  
@

<<confPlot2,fig.cap="Contraception example: comparison of point and confidence interval estimation for different methods.",fig.scap="Contraception comparison",echo=FALSE,fig.width=10,fig.height=10,warning=FALSE>>=
ggplot(contr_combCI, aes(var, est, colour=type, shape=model)) +
  geom_pointrange(aes(ymin=lwr, ymax=upr),
                  position=position_dodge(width=0.6)) +
  labs(x="", y="Estimate (log-odds of seropositivity)") +
  geom_hline(yintercept=0, lty=2) +
  coord_flip() +
  scale_colour_brewer(palette="Dark2", guide = revguide) +
  scale_shape(labels = mnamevec, guide = revguide) +
...
The point estimates and confidence intervals of the explanatory variables are similar across the six models, and across different methods for confidence interval construction, with a couple of exceptions.

The strongest predictors of contraceptive use among women in Bangladesh were urban residence and the presence of living children. Other factors, such as age, did not appear to have a meaningful effect.
...
## cm.cs <- glmer(use ~ cs(1 + age|district), Contraception, binomial, nAGQ=0,
##                  verbose = 1)
## cm.hetcs <- glmer(use ~ cs(1 + age|district, hom = FALSE), Contraception, binomial)
@

We can use \code{VarCorr} (and other accessor methods) as we would for models with default, unstructured covariance matrices.

<<cm_varcorr_view>>=
print(VarCorr(cm.diag))
@
