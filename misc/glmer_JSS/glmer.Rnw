...
Although the following analysis is not necessarily optimal for this data set, 
it serves to illustrate usage. The AR1 examples use a different grouping factor
to emphasize that AR1 structures are typically applied when time is the grouping 
factor.

TODO: write a better example since the variane covariance matrix below is just
of size 1
}

<<covar_cbpp>>=
gm1.diag <- glmer(incidence/size ~ period + treatment + avg_size 
                  + diag(1 | herd),
                  family = binomial, data = cbpp2, weights = size)
gm1.cs <- glmer(incidence/size ~ period + treatment + avg_size 
                + cs(1 | herd),
                family = binomial, data = cbpp2, weights = size)
gm1.homcs <- glmer(incidence/size ~ period + treatment + avg_size 
                   + cs(1 | herd, hom = TRUE),
                  family = binomial, data = cbpp2, weights = size)
gm1.ar1 <- glmer(incidence/size ~ treatment + avg_size + ar1(1 | period),
                family = binomial, data = cbpp2, weights = size)
gm1.hetar1 <- glmer(incidence/size ~ treatment + avg_size 
                 + ar1(1 | period, hom = FALSE),
                family = binomial, data = cbpp2, weights = size)
...
variance-covariance matrix for each grouping term using \code{VarCorr()}.
}
