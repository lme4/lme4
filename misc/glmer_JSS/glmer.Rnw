...
The \code{lme4} package support fitting negative binomial GLMMs, which represent an extension of GLMMs; the negative binomial family is within the
exponential dispersion family only if

\fixme{finish}

however, instead
of using \code{glmer}, the user must use the \code{glmer.nb} function 
instead. That being said, there are some parts in \code{glmer.nb} that
are still experimental and under development, whereas fitting negative binomial 
GLMMs are better implemented in \code{glmmTMB}.
