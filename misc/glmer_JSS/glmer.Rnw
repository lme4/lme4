...
\fixme{avoid needing plyr, reshape2 packages (melt/rename)}
...
<<varcorr_plot_gm1.ar1,fig.cap="CBPP example: variance-covariance matrix of the grouping term period.",fig.scap="CBPP VarCorr",echo=FALSE,eval=FALSE,fig.width=7>>=
# Removed since may not be that important or illustrative.
vc_mat2 <- melt(vc_mat$period)
ggplot(vc_mat2, aes(Var1, Var2, fill = value)) + 
  geom_tile() + 
...
contr_combCI <- merge(dcast(contr_combCI0, var + type + model ~ variable), combEst2, 
                by=c("var","model"), all.x=TRUE)
