...
\al{
Assuming that the form of the conditional distribution is from an exponential 
dispersion model (exponential, gamma, Poisson, binomial, and negative binomial),
we can weight observations differently by scaling the dispersion parameter, 
...
For GLMMs, the scale parameter is estimated outside the optimization of the 
likelihood over the $\beta$ and $\theta$ parameters. Specifically, it is 
estimated as the deviance divided by the residual degrees of freedom. 
...
Note that for Binomial and Poisson families, by default the scale is fixed at 1 
by assumption.
...
\al{When solving for the minimum of $Q(\bm u)$, there is a possibility for 
the proposed update for $\bm u$ has instead increased the value of the objective 
function $Q(\bm u)$. Let $Q(\bm u_i)$ represent the value of the objective 
function $Q(\bm u)$ at the current iterate $\bm u_{i}$, and similarly, let 
$Q(\bm u_{i-1})$ represent the the value of the objective function of the 
previous iteration. 

If $Q(\bm u_i) > Q(\bm u_{i-1})$, we proceed with the step-halving 
...
\end{equation}
and then compute $Q(\bm u_i^{(j)})$ at each step. The step-halving procedure 
...
\code{lme4} does support fitting negative binomial GLMMs, however, instead
