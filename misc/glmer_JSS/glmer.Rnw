...
Furthermore, \code{lme4} includes two variants of the \code{cbpp}
...
\al{Note that the \code{bobyqa} optimizer is the default for \code{glmer}.}
...
<<cbppModelII,cache=TRUE,dependson="newcbpp", warning=FALSE>>=
...
of \al{basic} plots (Figure~\ref{fig:glmerDiag}).
...
\al{
Additionally, \code{performance::check\_model()} checks for classical assumptions 
used for most linear models: normality of residuals, linear relationship, 
homogeneity of variance, and outliers. For mixed-effect models, it also checks 
for normality of random effects. The uncommonly used posterior predictive checks 
is meant to see whether the distributional family used fits well to the data. 
An example of a plot generated by \code{performance::check\_model()} can be found 
in Figure~\ref{fig:perf_chk_mod_cbpp}.
}

<<perf_chk_mod_cbpp,fig.cap="Model assumption checks using the performance package.",fig.height=10,echo=FALSE>>=
check_model(gm1, check = c("reqq", "binned_residuals", 
                                        "vif", "qq"))
@

\al{Another external package that is compatible with \code{merMod} objects is
\code{DHARMa}, which uses a simulation-based method for residual checks for 
generalized linear (mixed) models. Figure~\ref{fig:dharma_mod_cbpp} 
features some plots of the cbpp model. The left plot performs three tests, the Kolmogorov–Smirnov test, simulation-based dispersion tests, and simulation-based 
outlier tests. For more details see
\code{??DHARMa::testDispersion} and \code{DHARMa::testOutliers}.
}
...
\al{
Other two methods that are commonly employed to compute confidence intervals 
is the classical Wald method (\code{method = "Wald"}) and parametric 
bootstrapping method (\code{method = "boot"}), although the Wald method is used
for fixed effects parameters only.

All three of these confidence intervals, as well as the three different models, 
are compared against each other in Figure~\ref{fig:confPlot}.
}
...
\al{Another thing to consider is if the default optimizer, \code{bobyqa}, does not
...
\al{
Another feature of \code{lme4} is the ability to specify variance–covariance 
structures for (generalized) linear mixed models. As of version 2.0, 
\code{lme4} supports unstructured (general positive definite), diagonal, 
...
Here we illustrate fitting an AR1 model. Diagonal and compound symmetric
models are shown in the next example.
}
...
vc_mat <- VarCorr(gm.ar1)
@

\al{
To fit a heterogeneous structure, use \code{ar1(..., hom = FALSE)}. 
This isn't demonstrated here because it would cause a singular fit with 
the \code{cbpp} data set.

Exact details of the implementation can be found in the \code{lme4} covariance 
structure vignette. For now, we elaborate on how one can extract the 
variance-covariance matrix for the specific grouping term \code{period} using 
\code{VarCorr}.
}
...
<<graphContraception_loess, eval=FALSE>>=
xyplot(ifelse(use == "Y", 1, 0) ~ age|urban, Contraception,
       groups = livch, type = c("g", "smooth"), # smooth fits a loess curve.
...
<<graphContraception,echo=FALSE,fig.cap="Contraception example: proportion of contraceptive use by centered age, number of living children (line type), and urban/rural residence (facet). Curves fitted using GAM with cubic spline smoothing.">>=
ggplot(Contraception) + 
    ...
    scale_colour_discrete("Number\nof living\nchildren") 
@ 

<<range_contraception,eval=FALSE>>=
...
\al{We construct six different \code{glmer} models with varying choices of explanatory variables and grouping terms, and compare them using \code{anova}. 
...
<<cm1,message=FALSE,size='scriptsize'>>=
...
load(file="glmer_profbatch3.RData")
load(file="glmer_profbatch4.RData")
@
