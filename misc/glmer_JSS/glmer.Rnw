...
\fixme{add A Ly, get Walker current address, re-order Address section}
...
The \code{lme4} packages support fitting negative binomial GLMMs,
...
\code{performance} package and the \code{DHARMa} package, both are commonly 
used for model checking. The "n.s." in Figure~\ref{fig:dharma_mod_cbpp} means
"not significant."
...
<<dharma_mod_cbpp,fig.cap="Model assumption checks using the DHARMa package.",echo=FALSE>>=
...
profiles, each of which requires many $p-1$-dimensional optimizations. \al{The machinery regarding profiling for GLMMs is similar as LMMs. They are 
described in Section 5.1 of the \code{lmer} paper \citep{bates2015fitting}.}
...
\al{For demonstration purposes, we will show the results of one of the VarCorr objects. It is relatively similar for the other generalized linear mixed models.}

<<cm_varcorr_view>>=
vc.diag <- VarCorr(cm.diag)
