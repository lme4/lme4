...
\newcommand{\fixme}[1]{{\color{blue} FIXME: \textbf{#1}}}
...
\Plainauthor{Anna Ly, Rune Haubo Bojesen Christensen, Douglas Bates, Martin M\"achler, Benjamin M. Bolker}
...
which is recommended for large problems.} The mean, $\bm\mu_{\mc Y }$, of $\mc Y$ depends on
a linear predictor,
\begin{equation}
  \label{eq:GLMlinpred}
  \bm\eta=\bm X\bm\beta .
...
\fixme{explain/justify significant quantile test in DHARMa model}
...
\fixme{Figure~\ref{fig:cbppcompplot}: why are some profile results still wonky?}

<<cbppcompplot,fig.cap="CBPP example: comparison of point and confidence interval estimation for different methods.",fig.scap="CBPP comparison",echo=FALSE,fig.width=7,fig.height=5,warning=FALSE>>=
...
age (\code{I(age\^{}2)}) was added to account for the nonlinearity effect of age.
...
separating district-level from urban-within-district variation. The remaining model uses only the \code{urban:district} grouping. 
To reduce convergence warnings and facilitate interpretability, age (already approximately centered
in the data set) was standardized by scaling by the standard deviation.
...
\fixme{make this into a proper table?}
...
\fixme{plot of predicted effects? (parallel Fig 10)}
...
<<contracompplot,fig.cap="Contraception example: comparison of point and confidence interval estimation for different methods.",fig.scap="Contraception comparison",echo=FALSE,fig.width=10,fig.height=10,warning=FALSE,fig.lab="fig:contracomp">>=
ggplot(combCI, aes(var, est, colour=type, shape=model)) +
  geom_pointrange(aes(ymin=lwr, ymax=upr),
...
Thus instead of (e.g.) \code{(1+urban|district)}, we can specify the random effect as
\code{diag(1+urban|district)} (diagonal, heterogeneous variances);
\code{diag(1+urban|district, hom = TRUE)} (diagonal, homogeneous variances);
\code{cs(1+urban|district)} (compound symmetric, heterogeneous variances); or
\code{cs(1+urban|district, hom = TRUE)} (compound symmetric, homogeneous variances).
...
\fixme{Doug: do we (i.e. you) want to add this?}
