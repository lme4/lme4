...
Subjects with multiple measurements at same timepoint, errors autocorrelated over time on subject leve:
````{r}
set.seed(2352)
nsubj <- 50
nreps <- 1
...
(p5 <- qplot(y=mu, x=time, data=fr, geom="line") + facet_wrap(~id) + 
	geom_line(aes(y=y, col=rep), lwd=.5))
````
...
... scale of AR(1) seems to get overestimated ...?!? 
