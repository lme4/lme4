...
The unconditional distribution of the random effects, $\mc B\sim\mc N(\bm 0, \bm\Sigma)$, and the definitions
of the spherical random effects, $\mc U$, such that $\mc B=\bm\Lambda_{\bm\theta}\mc U$ where
...
\item Like a GLM, the definition of a GLMM incorporates a distribution family and a link function.
  The distribution family determines the unit deviances, $d(y_i,\mu_i),i=1,\dots,n$. Distributions in the exponential distribution family have a canonical link which is the default link for those families.
...
\item For given values of $\bm\beta$ and $\bm\theta$, the penalized sum of unit deviances (PSUD),
  $\sum_{i=1}^n d(y_i,\mu_i)+\|\bm u\|^2$, is minimized with respect to $\bm u$.
...
\item In certain simple models adaptive Gauss-Hermite quadrature (aGHQ) can be used to
  approximate the log-likelihood more accurately, at the expense of more evaluations of the PSUD.
...
\item Let 
  \begin{equation}
  Q(\bm u) = \left\|\bm W^{1/2}\left(\yobs-\bm\mu_{\mc
