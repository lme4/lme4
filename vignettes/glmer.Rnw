...
\Abstract{%
  ...
  the MLE for a given set of parameters (by default we use the
...
The vector $\bm\eta=\bm X\bm\beta$ is called the \emph{linear predictor} which is mapped to
...
The distribution family determines the \emph{unit deviance} function, $d(y_i,\mu_i)$, and the mle's
...
negative twice the log-likelihood at $(y_i, y_i)$ (which is generally but not always zero).
...
Fortunately, there is a fast, robust algorithm, \emph{iteratively reweighted least squares} (IRLS),
to do this. 
...
These are the identity for the Gaussian family, the \emph{logit} or ``log-odds'' function,
...
General nonlinear optimizers can then be used to determine the mle's for all the parameters.
