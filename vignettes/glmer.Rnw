...
For simple models involving scalar random effects from a single random-effects term \bmb{can we, in principle, deal with nested random effects as well?} the multivariate integral
...
\item For distributions in the exponential distribution family we usually use the canonical link.
...
\item In certain simple models a more accurate method, adaptive Gauss-Hermite quadrature, can be used to
  approximate the log-likelihood.
\item If the PSUD is minimized with respect to $\bm u$ and $\bm\beta$ simultaneously then the approximate
  profiled log-likelihood becomes a function of $\bm\theta$ only. \bmb{is it worth expanding on this slightly: ``as discussed further below, this approximation can save computation time for large problems'' (or some such)? (See [currently] line 195 or thereabouts; also, Alex Stringer requested more detail on this approach \ldots}
\end{enumerate}
