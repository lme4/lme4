...
To use adaptive Gauss-Hermite quadrature for model fitting 
in \code{glmer} models, users would set the argument \code{nAGQ}, the 
number of quadrature points, to a value greater than 1.
...
The only difference is that by default \code{glmer} uses a two-step fitting procedure, using \code{nAGQ=0} at the first stage to get preliminary estimates which are then used as starting points for a second optimization with Laplace approximation or Gauss-Hermite quadrature as specified by the user. Different nonlinear optimizers can be used at each stage: the current default, as specified in \code{glmerControl}, is to use Powell's BOBYQA followed by a box-constrained variant of the Nelder-Mead simplex algorithm. For faster, approximate fitting, the second stage can be omitted; in the rare cases where the initial \code{nAGQ=0} fit gives poor results, the first stage can be skipped via \code{glmerControl(nAGQ0initStep = FALSE)}.
...
compound symmetric, homogeneous variances (\code{cs(1+urban|district, hom = TRUE)}).
