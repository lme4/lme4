...
For a generalized linear mixed model, the variance-covariance matrix is still
constructed via a relative co-factor $\Lambda_{\theta}$, however, there is
no scale parameter $\sigma$. That is, the variance-covariance matrix is
constructed using
\[
\mathbf{\Sigma}_{\mathbf{\theta}} = \Lambda_{\theta} 
  \Lambda_{\theta}^{T}
\]
instead.
