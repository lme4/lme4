...
The first half of this vignette provide a more detailed exp1lanation of how the 
...
matrix as specified as $Var(\mathbf{b}$. Typically, we denote the 
...
inear mixed model. In `lme4`, the variance-covariance matrix is constructed by
...
One of the **major differences** between the four covariance classes (`Covariance.us` (unstructured), `Covariance.diag` (diagonal), `Covariance.cs` (compound symmetry), and `Covariance.ar1` (autoregressive order 1)) **is the construction of the the relative co-factor** $\Lambda_{\theta}$.
...
The next three covariance structures can either be heterogenous or homogenous.
...
```{r}
lme4:::getPar(fm1.cs@reCovs[[1]])
lme4:::getTheta(fm1.cs@reCovs[[1]])
lme4:::getLambda(fm1.cs@reCovs[[1]])
```
...
When constructing generalized linear mixed models, `lme4` uses the Cholesky decomposition to reduce the problem to a function of the covariance parameters. In particular, `lme4` parameterizes the random-effects variance–covariance matrix $$
