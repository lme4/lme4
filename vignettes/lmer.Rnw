...
\shortcites{bolker_strategies_2013,sleepstudy}
...
library(knitr)
library(lme4)
## library(lme4pureR)  ## don't load; masks lme4::lmer, causes trouble
library(ggplot2)
theme_set(theme_bw())
library(grid)
library(lattice)
...
%\scwViaDb{Note about academic honesty: all text in this magenta colour
%  was directly stolen (or liberally absorbed) by SCW from the vast
%  amount of material written by DB on \pkg{lme4}.}

The \pkg{lme4} package for \proglang{R} provides functions to fit and analyze
...
incorporates both fixed- and random-effects terms in a
...
computation, we have developed several additional frameworks
that draw on the basic ideas of \code{lme4} but modify its
structure or implementation in various ways. These descendants include the
\mbox{\pkg{MixedModels}} package in \proglang{Julia}, the
\pkg{lme4pureR} package in \proglang{R}, and the \code{flexLambda}
development branch of \pkg{lme4}.
The current article is largely restricted to describing
of the current stable version of the \pkg{lme4} package (1.1-7), with
Appendix~\ref{sec:modularExamples} describing hooks into the
computational machinery that are designed for extension development.
% , as has been done for example in the \pkg{gamm4} \citep{gamm4} and
% \pkg{blme} \citep{blme} packages.

Another goal of this article is to contrast the approach used by
\pkg{lme4} with previous formulations of mixed models. 
The expressions for the profiled log-likelihood and profiled REML
...
Nonetheless there are subtle but important changes in the
formulation of the model and in the structure of the resulting
penalized least squares (PLS) problem to be solved
(Section~\ref{sec:PLSpureR}).  We derive the
current version of the PLS problem (Section~\ref{sec:plsMath}) and
contrast this result with earlier formulations
(Section~\ref{sec:previous_lmm_form}).
%% \bmb{do we need this last sentence here, or can it wait?}
%% One specific change is that
%% both the residual covariance matrix and the random effects covariance
%% matrix are scaled by a parameter, $\sigma^2$ (Section~\ref{sec:LMMs}).
%% A consequence of this difference is that a model in which the residual
%% variance is numerically zero does not exist in the \pkg{lme4}
%% framework.  As we discuss, excluding this subset of models increases
%% the stability of our model fitting algorithms
%% (Sections~\ref{sec:stability}, \ref{sec:previous_lmm_form}).

This article is organized into four main sections (\ref{sec:lFormula};
\ref{sec:mkLmerDevfun}; \ref{sec:optimizeLmer}; \ref{sec:mkMerMod}),
each of which corresponds to one of the four largely separate modules
that comprise \pkg{lme4}.  Before describing the details of
each module, we describe the general form of the
linear mixed model underlying \pkg{lme4} (Section~\ref{sec:LMMs});
introduce the \code{sleepstudy} data that will be used as an example
throughout (Section~\ref{sec:sleepstudy}); and broadly outline \pkg{lme4}'s
...
\begin{itemize}
%% \item As mentioned in the introduction and described in greater detail
%%   below (Section~\ref{sec:stability}), \pkg{lme4} differs from other
%%   mixed-model formulations in that $\sigma$ scales the covariance
%%   matrices for both the conditional distribution of $\mc{Y}$ given
%%   $\mc{B}$ (Eq.~\ref{eq:LMMcondY}) and the marginal distribution of
%%   $\mc{B}$ (Eq.~\ref{eq:LMMuncondB}). \bmb{how important is this?}
\item This formulation of linear mixed models allows for a relatively
  ...
  (Section~\ref{sec:profdev}, Eq.~\ref{eq:profiledDeviance}).
\item The matrices associated with random effects, $\bm Z$ and $\bLt$,
  ...
  \ref{sec:CSCmats}
  %, and \ref{sec:RSCmatChol}
  provide details on
  these structures, and how to represent them efficiently.
\item The interface provided by \pkg{lme4}'s \code{lmer} function 
  is slightly less general than the model described by equations
  \ref{eq:LMMcondY}, \ref{eq:LMMuncondB}, and \ref{eq:relcovfac}.  To
  ...
  experimental \code{flexLambda} branch of \pkg{lme4} on \github.
\end{itemize}

%% This is the general form of linear mixed models that can be fitted by
%% the \pkg{lme4} package.  The unknown parameters of this model are
%% $\bm\beta$, $\thetavec$, and $\sigma^2$, \pkg{lme4} can estimate by
%% either maximum likelihood or restricted maximum likeihood.  We
%% describe these model fitting proceedures in Sections ??.  The known
%% inputs of this model are $\bm o$, $\bm W$, $\bm X$, $\bm Z$, and the
%% nature of the mapping from $\thetavec$ to $\bm\Lambda$.  The first three
%% inputs are specified as they are in the \code{lm} function.  The
%% latter two inputs are more interesting, and reflect the increased
%% complexity associated with mixed-effects models. We describe how to
%% specify these elements using \proglang{R} model formulas in Sections
%% ??.

%% \subsection{Sparse matrix structure}

%% Although all models of the form ?? above can be fitted by the basic
%% machinary underlying \pkg{lme4}, the \code{lmer} interface puts some
%% restrictions on the structure of the $\bm Z$ and $\bLt$
%% matrices.  The structure of $\bm Z$ in particular is based on $k$
%% model matrices $\bm Z_1, \bm Z_2, ..., \bm Z_k$.  The dimensions of
%% the $i$th model matrix, $\bm Z_i$, are $n \times q_i$.  The $\sum_i
%% q_i$ columns give explanatory variables associated with a particular
%% random effect.  These model matrices are Random effects from the same
%% model matrix are correlated, where
...
\code{Day} to each \code{Subject} (Fig. \ref{fig:sleepPlot}),
<<sleep>>=
str(sleepstudy)
@
...
REML criterion (Eq.~\ref{eq:REMLdeviance}) using
%% BMB: think we can do without the output here??
...
\scwViaDb{Like most model-fitting functions in \proglang{R},
...
%% Two questions about this formula structure immediately arise.  What
%% is the meaning of the vertical bar?  Why would one require multiple
...
with the standard \proglang{R} modelling paradigm
\citep{Chambers:1993}.
...
\proglang{R} modelling functions (e.g. \code{lm} or \code{glm}). The
...
this a special kind of interaction is that these effects are modelled
...
(e.g. \citet{henderson_1982}; \citet{gelman2005analysis}).  For
...
(e.g. \citet{1977EfronAndMorris}), or account
for lack of independence in the residuals due to block structure or
repeated measurements (e.g. \citet{laird_ware_1982}).

%% or to estimate regression coefficients when the number of columns in
%% the model matrix is larger than the number of samples (REFS).

%% Multiple random-effects terms are useful for two principal reasons.
%% First, they allow one to group data using more than one non-nested
%% factors.  Such flexibility is required to fit several types of models.
%% For example, item response models require both \code{item} and
%% \code{subject} grouping factors, such that nearly every subject is
%% tested on nearly every item.  Such factors are non-nested and so would
%% require two random effects terms.  Second, two random effects terms
%% with the same grouping factor allow one to model the independent
%% random effects.
...
Our description of this model incorporates any non-zero mean of the
...
%% In terms of the mathematical formulation of the \code{lmer} model, the
%% mean is the only element in the vector of fixed-effects coefficients,
%% $\bm\beta$, and the standard deviation is the only element in the
%% vector of covariance parameters, $\bm\theta$.
...
random effects terms, \code{x+(1|g)+(0+x|g)}, as in the final example
of Table~\ref{tab:formulas}.  For example, from the printout of model
\code{fm1} of the \code{sleepstudy} data
(Section~\ref{sec:sleepstudy}), we see that the estimated correlation
between the slope for \code{Days} and the intercept is fairly low
(\Sexpr{round(attr(VarCorr(fm1)$Subject, "correlation")[2],3)}) % $
(See
Section~\ref{sec:summary} below for more on how to extract the random
effects covariance matrix.)  We may use the double-bar notation to
fit a model that excludes a correlation parameter:
<<uncorrelatedModel,results="hide">>=
fm2 <- lmer(Reaction ~ Days + (Days || Subject), sleepstudy)
@
%% Note the expansion of the formula in this printout.

%%<<howTheDoubleBarFormulaIsParsed>>=
%%formula(fm2)
%%@

Although mixed models where the random
slopes and intercepts are assumed independent
are commonly used to reduce the complexity
of random-slopes models, they do have one subtle drawback.
Models in which the slopes and intercepts are allowed
to have a non-zero correlation
(e.g. \code{fm1}) are invariant to additive shifts of the
continuous predictor (\code{Days} in this case).
This invariance breaks
down when the correlation is constrained to zero;
any shift in the predictor will necessarily lead to a change
in the estimated correlation, and in the likelihood and
predictions of the model.
For example, we can eliminate the correlation in
\code{fm1} simply by shifting \code{Days} by an amount equal to the
ratio of the estimated among-subject standard deviations 
multiplied by the estimated
correlation (i.e., $\sigma_{\text{\small slope}}/\sigma_{\text{\small intercept}} \cdot \rho_{\text{\small slope:intercept}}$).
The use of such models should ideally be restricted to cases where
the predictor is measured on a ratio scale (i.e. the zero
point on the scale is meaningful, not just a location defined
...
%% \scw{FIXME: conclusion statement here about an overall lesson}

%% \bmb{I question whether we want to do this, in light of Rune's
%%   demonstration that we are essentially setting up a model that is no
%%   longer invariant to recentering of the \code{Days} parameter \ldots
%%   ? Should we at least warn?  Is there a better illustration of
%%   double-bar notation?}

%% \scw{I've tried to address this issue with the above paragraph.
%%   However, this raises some questions for me.  For example, does this
%%   have implications for degrees of freedom/parameter counting?  More
%%   importantly, as you also ask, is there a better illustration of
%%   double-bar?  It would be nice to do uncorrelated random effects
%%   associated with levels of a factor (i.e. factor to the left of the
%%   bar), but as I discovered last week, expandDoubleVert does not work
%%   for this case. ... after some thought, I guess the best example is
%%   two uncorrelated slopes -- too bad sleepstudy doesn't do that.}
...
$\bm Z$ (Eq.~\ref{eq:LMMcondY}; Section~\ref{sec:mkZ}), and the
structure of the relative covariance factor, $\bLt$
(Eq.~\ref{eq:relcovfac}; Section~\ref{sec:mkLambdat}), which are
...
$\bLt$.  % Below in Section~\ref{sec:SCSmats} we
% discuss efficient strategies for storing and computing with these
% matrices.
...
\begin{table}[tb]
  ...
  \begin{tabular}{lll}
    ...
    $q$ & Number of columns of random effects model matrix, $\bm Z$ \\
...
\begin{table}[tb]
  ...
  \begin{tabular}{lll}
    ...
    \hline
  \end{tabular}
  \caption{Symbols used to describe the structure of the random
    effects model matrix and the relative covariance factor. The
    subscript $i = 1, \dots, k$ denotes a specific random effects term.}
  \label{tab:algebraic}
\end{table}
...
described below in Eq.~\ref{eq:blockCholeskyDecomp}). This reduction
in fill-in provides more efficient matrix operations within the
penalized least squares algorithm (Section~\ref{sec:plsMath}).
%\bmb{in the document I got this from (\url{http://lme4.r-forge.r-project.org/slides/2009-07-01-Lausanne/6TheoryD.pdf} p. 20), the first matrix is expressed as $\bm U'\bm U + \bm I$.  Is there any point in adding the notation $\bm U \equiv \bm Z\bLt$?  No.}
...
$\bm J_i$.  Similarly for the subsequent blocks.  With these
...
%% \begin{equation}
%%   \label{eq:12}
%%   \bm Z_i = \left(\bm J_i\trans * \bm X_i\trans\right)\trans,
%% \end{equation}
\begin{equation}
  \label{eq:Zi}
  ...
  \end{bmatrix}
\end{equation}
where $*$ and $\otimes$ are the Khatri-Rao \citep{khatri1968solutions}
and Kronecker products, and $\bm J_{ij}\trans$ and $\bm X_{ij}\trans$
are row vectors of the $j$th rows of $\bm J_i$ and $\bm X_i$.  These rows correspond
to the $j$th sample in the response vector, $\mc Y$, and thus $j$ runs
from $1 \dots n$.  The \pkg{Matrix} package for \proglang{R} contains
a \code{KhatriRao} function, which can be used to form $\bm Z_i$.  For
example, if we begin with a raw model matrix,
<<rawModelMatrix>>=
...
%%   \begin{split}
  %% Z_i\trans Z_i = &
  %%   \begin{bmatrix}
  %%   \bm J_{i1} \otimes \bm X_{i1} &
  %%   \bm J_{i2} \otimes \bm X_{i2} &
  %%   \hdots &
  %%   \bm J_{in} \otimes \bm X_{in} \\
  %% \end{bmatrix}
  %%   \begin{bmatrix}
  %%   \bm J_{i1}\trans \otimes \bm X_{i1}\trans \\
  %%   \bm J_{i2}\trans \otimes \bm X_{i2}\trans \\
  ...
  %% \end{bmatrix} \\
  %% = &
  %% (\bm J_{i1} \otimes \bm X_{i1}) (\bm J_{i1}\trans \otimes \bm
  %% M_{i1}\trans) +
  %% (\bm J_{i2} \otimes \bm X_{i2}) (\bm J_{i2}\trans \otimes \bm
  %% M_{i2}\trans) + \hdots +
  %% (\bm J_{in} \otimes \bm X_{in}) (\bm J_{in}\trans \otimes \bm
  %% M_{in}\trans) \\
  %% = &
  %% \bm J_{i1} \bm J_{i1}\trans \otimes \bm X_{i1} \bm X_{i1}\trans +
  %% \bm J_{i2} \bm J_{i2}\trans \otimes \bm X_{i2} \bm X_{i2}\trans +
  %% \hdots +
  %% \bm J_{in} \bm J_{in}\trans \otimes \bm X_{in} \bm X_{in}\trans \\
  %% = &
  %% \sum_{j=1}^n \bm J_{ij} \bm J_{ij}\trans \otimes \bm X_{ij} \bm X_{ij}\trans
%%   \end{split}
%% \end{equation}
...
%%   \bm Z =
%%   \begin{bmatrix}
%%     \left(\bm J_1\trans * \bm X_1\trans\right)\trans &
%%     \left(\bm J_2\trans * \bm X_2\trans\right)\trans &
%%     \hdots &
%%     \left(\bm J_k\trans * \bm X_k\trans\right)\trans \\
%%   \end{bmatrix}
%% \end{equation}
%% The Khatri-Rao product is sometimes referred to as a column-wise
%% Kronecker product, because,
%% \begin{equation}
%%   \label{eq:2}
%%   \bm A * \bm B =
%%   \begin{bmatrix}
%%     \bm a_1 \otimes \bm b_1 &
%%     \bm a_2 \otimes \bm b_2 &
%%     \hdots \\
%%   \end{bmatrix}
%% \end{equation}
%% where $\bm a_i$ and $\bm b_i$ are the $i$th columns of $\bm A$ and $\bm
%% B$.
By transposing Eq.~\ref{eq:Z} and substituting in Eq.~\ref{eq:Zi}, we
...
%%     \bm J_1\trans * \bm Z_1\trans & \text{term 1} \\
%%     \bm J_2\trans * \bm Z_2\trans & \text{term 2} \\
%%     \vdots \\
%%   \end{blockarray}
%% \end{equation}
\begin{equation}
...
this sparsity, for example by using sparse matrix methods, when fitting mixed
models (Section~\ref{sec:CSCmats}).

%% Note that $\bm Z\trans$ contains $n\sum_{i=1}^k p_i$ potentially
%% non-zero values and $n\sum_{i=1}^k p_i(\ell_i - 1)$ structural zeros.
%% that each Kronecker product associated with the $i$th term is a
%% column vector containing $p_i$ potentially non-zero values and
%% $p_i(\ell_i-1)$ structural zeros. Thus
...
numeric vector containing the non-zero elements). This template
...
@
For a model with a single random effects term,
$\bm\Lambda_i\trans$ would be the initial transposed relative
covariance factor itself.
%% For multiple terms, the relative covariance factor, $\bLt$,
%% is a block diagonal matrix whose $i$th block, $\bm\Lambda_i$, is of
%% size $q_i$, $i = 1,...,k$.  These $\bm\Lambda_i$ matrices are combined
%% using \code{.bdiag} to construct the complete relative covariance
%% factor.

The transposed relative covariance factor, $\bLt\trans$, that arises from parsing the
formula and data is set at the initial value of the covariance
parameters, $\bm\theta$.  However, during model fitting, it needs to
be updated to a new $\bm\theta$ value at each iteration (see
...
@
Section~\ref{sec:plsI} describes the process of
...
(Eqs.~\ref{eq:LMMcondY}, \ref{eq:LMMuncondB}, and \ref{eq:relcovfac}),
the covariance parameter vector, $\bm\theta$, appears only in the
marginal distribution of the random effects (Eq.~\ref{eq:LMMuncondB}).
...
0,\bm\Sigma_{\bm\theta})$ distribution (Eq.~\ref{eq:LMMuncondB}).
Although it may seem more natural to define $\mc U$ in terms of $\mc
B$ we must write the relationship as in \eq{eq:BequalsLamU} to allow
for singular $\bLt$. The conditional distribution
(Eq.~\ref{eq:LMMcondY}) of the response vector given the random
...
mixed model involve repeated applications of the method of penalized
least-squares (PLS). In particular, the penalized least squares
problem is to minimize the penalized weighted residual sum-of-squares,
\begin{equation}
  \label{eq:pwrss}
...
the term, $\left\| \bm u \right\|^2$, in Eq.~\ref{eq:pwrss} penalizes models with
larger magnitude values of $\bm u$.
% \begin{equation}
%   \label{eq:PLSprob}
%   \bm\mu_{\mc U|\mc Y=\yobs}=\arg\min_{\begin{bmatrix} \bm u \\
%   \bm\beta \\ \end{bmatrix}} r^2(\bm\theta, \bm\beta, \bm u)
% .
% \end{equation}%\end{linenomath}

% In the so-called ``pseudo-data'' approach to solving such penalized
% least squares problems we write the objective as a residual
% sum-of-squares for an extended response vector and model
In the so-called ``pseudo-data'' approach
we write the penalized weighted residual sum-of-squares as the squared
length of a block matrix equation,
\begin{equation}
...
$\begin{bmatrix}
  ...
  \widehat{\bm\beta}_{\bm\theta} \\ \end{bmatrix}$, follows the
normal equations,
\begin{equation}
...
The cross-product matrix in Eq.~\ref{eq:normalEquations} can be
Cholesky decomposed,
\begin{equation}
...
% \begin{equation}
%   \label{eq:pwrssBlockIdentity}
%   r^2(\thetavec,\bm\beta,\bm u) =
%   r^2(\thetavec,\widehat{\bm\beta}_{\bm\theta},\bm\mu_{\mc U|\mc Y = \yobs}) +
%   \left\|   \begin{bmatrix}
%     \bm L\trans_{\bm\theta} & \bm R_{ZX} \\
%     \bm 0 & \bm R_{X} \\
%   \end{bmatrix}
%   \begin{bmatrix}
%   \bm u - \bm\mu_{\mc U|\mc Y = \yobs} \\
%   \bm\beta - \widehat{\bm\beta}_{\bm\theta} \\
%   \end{bmatrix}
%   \right\|^2 ,
% \end{equation}
% and as,
...
The integral in Eq.~\ref{eq:densityY} may be more explicitly written
as,
\begin{equation}
...
\end{equation}
% This is the probability density used to form the log likelihood
% (Eq.~\ref{eq:likelihood}).

\subsection{Evaluating and profiling the deviance and REML criterion}
\label{sec:profdev}
...
of the log-likelihood (Eq.~\ref{eq:likelihood}), to find a compact
...
least-squares problem in Eq.~\ref{eq:pwrssPseudoData}. Therefore we
...
The REML criterion can be obtained by
integrating the marginal density for $\mc Y$ with respect to the fixed
effects \citep{laird_ware_1982},
\begin{equation}
...
Eq. 6 of \citet{henderson_1982} and would be expressed as,
\begin{equation}
  \label{eq:henderson}
  \begin{bmatrix}
...
The primary difference between Eq.~\ref{eq:henderson} and
Eq.~\ref{eq:batesDebRoy} is the order of the blocks in the system matrix.
...
arrangement in Eq.~\ref{eq:batesDebRoy} is to allow for evaluation of
...
this matrix is the first block to be factored.  If $\bm X\trans\bm
X$ is the first block to be factored it is much more difficult to
preserve sparsity.
...
Eq.~\ref{eq:batesDebRoy} cannot be used in these cases because
...
Eq.~\ref{eq:batesDebRoyprofdev}.  These expressions can be translated
into the current formulation.  From Eq.~\ref{eq:profiledDeviance} we can see
...
% \scw{should we use
%   tildes?  Either $\tilde{\bm u}$ or $\bm\mu_{\mc U|\mc Y=\yobs}$}

% \scw{Should have a short concluding paragraph here summing up the
%   usefulness (or lack thereof) of these gradient expressions.  They are used for certain model types in the Julia implementation and do help.}

%% In the next section we describe how random-effects terms in a model
%% formula determine sections of $\bm Z$ and $\bLt$ along with the
%% mapping, $\bm\theta\rightarrow\bLt$.  For models that can be
%% described by random-effects terms in a model formula,
%% $\partial\bLt/\partial\theta_i$ will be a sparse binary matrix for
%% all $i$.  This provides straightforward evaluation of
%% Eq.~\ref{eq:gradr2} but evaluation of Eq.~\ref{eq:graddet}
%% remains problematic in the general case.  In certain special cases
%% it is possible to use aspects of the structure of the matrices in
%% question to provide effective methods.


\subsection{Penalized least squares algorithm}
\label{sec:PLSpureR}

%% We now describe code for implementing the PLS algorithm.
For efficiency, in \pkg{lme4} itself, PLS is implemented in compiled
\proglang{C++} code using the \pkg{Eigen} templated \proglang{C++}
...
requires many iterations. % In general, we
% stress language-agnosticism to emphasize that our interests in PLS
% extend beyond \pkg{lme4} itself.

The PLS algorithm takes a vector of covariance parameters,
$\bm\theta$, as inputs and returns the profiled deviance (Eq.
\ref{eq:profiledDeviance}) or the REML criterion
(Eq. \ref{eq:REMLprofiled}). This PLS algorithm consists of four main
steps:
\begin{enumerate}
...
\begin{table}[tb]
  ...
  \begin{tabular}{p{2.2in}llp{2in}}
    ...
    Fixed effects model matrix & \code{X} & $\bm X$ (Eq.~\ref{eq:LMMcondY}) & double dense\footnote{In previous versions of \pkg{lme4} a sparse $X$ matrix, useful for models with categorical fixed-effect predictors with many levels, could be specified; this feature is not currently available.}
    matrix \\
    ...
    (Eq.~\ref{eq:LMMcondY}) & double diagonal matrix \\
    Offset & \code{offset} & $\bm o$ (Eq.~\ref{eq:LMMcondY}) & double vector \\
    \hline
  \end{tabular}
...
\begin{table}[tb]
  ...
  \begin{tabular}{p{2.2in}llp{2in}}
    ...
    \hline
  \end{tabular}
  \caption{Quantities updated during an evaluation of the LMM
    objective function.}
  \label{tab:updatePLS}
\end{table}
...
$\bm\theta$.  The updated $\bLt$ is then used to update the random
effects Cholesky factor, $\bm L_{\bm\theta}$
(Eq.~\ref{eq:blockCholeskyDecomp}).  The mapping from the covariance
...
of the non-zero elements of $\bLt$.  
...
returns a numeric vector. The non-zero elements of sparse matrix
classes in \pkg{Matrix} are stored in a slot called \code{x}.

In the current version of \pkg{lme4} (v. 1.1-7) the mapping from
$\bm\theta$ to $\bLt$ is represented as an \proglang{R} integer vector,
\code{Lind}, of indices, so that
%\code{Lind} has length equal to the number of non-zero
%elements of $\bLt$,
<<PLSupdateThetaWithLind>>=
mapping <- function(theta) theta[Lind]
@
The index vector \code{Lind} is computed during the model setup and stored in
the function's environment.  Continuing the example from
...
For example, Appendix~\ref{sec:homoVar} shows how to use \code{Lind}
to fit a model where a random slope and intercept are uncorrelated and
...
allows the capabilities for a completely general mapping.  This added
...
is to solve the normal equations (Eq.~\ref{eq:normalEquations}) for the
...
factorization (Eq.~\ref{eq:blockCholeskyDecomp}).  In a complex model
fit to a large data set, the dominant calculation in the evaluation of
the profiled deviance (Eq.~\ref{eq:profiledDeviance}) or REML criterion
(Eq.~\ref{eq:REMLprofiled}) is this sparse Cholesky factorization
(Eq.~\ref{eq:blockCholeskyDecomp}).  The factorization is performed in two
...
\begin{align}
    ...
    \bm L\trans\bm_{\bm\theta} \bm P\bm u&=\bm c_u-\bm R_{ZX}\widehat{\bm\beta}\label{eq:tildeu}
\end{align}
...
%% The CHOLMOD library of C functions
...
%% the model.  Furthermore, because CHOLMOD functions allow for updating
%% $\bm L_{\bm\theta}$ directly from the matrix $\bLt\trans\bm Z\trans$
%% without actually forming $\bLt\trans\bm Z\trans\bm Z\bLt+\bm I$ we
%% generate and store $\bm Z\trans$ instead of $\bm Z$ (note that we have
%% ignored the weights matrix, $\bm W$, for simplicity).  As described
%% below, we can update $\bLt\trans\bm Z\trans$ directly from $\thetavec$
%% without forming $\bLt$ and multiplying two sparse matrices.  In the
%% \proglang{Julia} code we use the direct update of $\bLt\trans\bm
%% Z\trans$.  In the \proglang{R} code we first update $\bLt\trans$ then
%% form the sparse product $\bLt\trans\bm Z\trans$.  A third alternative
%% is to compute and save the cross-products of the model matrices, $\bm
%% X$ and $\bm Z$, and the response, $\bm y$, before starting the
%% iterations. To allow for case weights, we save the products $\bm
%% X\trans\bm W\bm X$, $\bm X\trans\bm W\bm y$, $\bm Z\trans\bm X$, $\bm
%% Z\trans\bm y$ and $\bm Z\trans\bm W\bm Z$.

%% However, note that the
%% advantage of saving these products is lost if $\bm W$ can change, as
%% in the Penalized Iteratively-Reweighted Least Squares (PIRLS)
%% algorithm for fitting generalized linear mixed models (GLMMs).
%% \bmb{Cite the glmer paper?}
...
%% From \eq{eq:normalEquations} we can see that the conditional mean
%% satisfies%\begin{linenomath}
%% \begin{equation}
%%   \label{eq:PLSsol}
%%   \left(\bLt\trans\bm Z\trans\bm W\bm Z\bLt+\bm I_q\right)
%%   \bm\mu_{\mc U|\mc Y=\yobs}=\bLt\trans\bm Z\trans\bm W(\yobs-\bm o-\bm X\widehat{\betavec}_\thetavec),
%% \end{equation}%\end{linenomath}
...
%% be used to solve Eq.~\ref{eq:PLSsol}.
...
%% not depend on $\thetavec$ itself, and the \emph{sparse Cholesky
%%   factor}, $\bm L_\thetavec$, which is a sparse, lower-triangular matrix
%% such that%\begin{linenomath}
%% \begin{equation}
%%   \label{eq:sparseChol}
%%   \bm L_\thetavec\bm L\trans_\thetavec=\bm P
...
(Eq. \ref{eq:meanYgivenU}), and the weighted residuals with new values for
...
%% \subsection{Computational details and methods}
...
squares (PLS) problem (\ref{eq:normalEquations}) must be solved at each
evaluation of the objective function during the optimization
...
and calculations involving the sparse matrices associated with random
effects terms (Sections~\ref{sec:LMMmatrix}).
...
for separation of the symbolic and numeric phases.  Thus we perform
the symbolic phase as part of establishing the structure representing
the model (Section~\ref{sec:lFormula}).  Furthermore, because CHOLMOD
...
(eqn.~\ref{eq:blockCholeskyDecomp}) efficiently.  The key to doing so
...
where the row position, column position and element value the nonzeros
...
compressed sparse row (CSR)~\citep[Ch.~2]{davis06:csparse_book}. The
CHOLMOD (and, more generally, the SuiteSparse package of C libraries)
uses the CSC storage format.  In this format the non-zero elements in
...
each column of matrices derived from $\bm Z\trans$ (such as $\bLt\trans\bm Z\trans\bm W^{1/2}$) is constant, the vector of nonzeros in the
...
algorithm adapted from Steven Johnson's \code{NLopt} C library
\citep{NLopt} and a wrapper for Powell's BOBYQA
algorithm, implemented in the \pkg{minqa} package \cite{minqa_pkg}
as a wrapper around
Powell's original FORTRAN code \citep{Powell_bobyqa}.  More generally,
...
package \citep{optimx_pkg}, although the only optimizers provided via \pkg{optimx} that
satisfy the constraints above are the \code{nlminb} and
\code{L-BFGS-B} algorithms that are themselves wrappers around the
versions provided in base R's \code{optim} function.  Given a simple
wrapper function, Several other algorithms from Steven Johnson's
\code{NLopt} package are also available via the \pkg{nloptr} wrapper
package (e.g., alternate implementations of Nelder-Mead and BOBYQA,
and Powell's COBYLA algorithm).
...
\cite{pinheiro_unconstrained_1996}), we instead use the Cholesky
...
%\bmb{is this
%  (especially ``only if'') true? As off-diagonal elements go to $\pm
%  \infty$ we \emph{approach} singularity, but I think the statement is
%  still technically correct \ldots} \bmb{As mentioned above??}
...
(Eqs.~\ref{eq:grad}--\ref{eq:gradr2}), but in general we will have to
...
scaled or centred data --- we are working to implement appropriate
...
represented as objects of the \code{S4} class \code{lmerMod}.  We
...
\end{equation}
which follows from the Schur complement identity (Theorem 1.2 of \citet{zhang2006schur}).  This
...
The square-root diagonal of this covariance matrix, $\sqrt{\Var_{\bm\theta,
  \sigma}(\hat\betavec)}$, contains the estimates of the standard errors
of fixed effects coefficients.  These standard errors are used to
construct Wald confidence intervals with \code{confint(., method =
  "Wald")}.  Such confidence intervals are approximate, and depend on
the assumption that the likelihood profile of the fixed effects is
linear on the $\zeta$ scale (Section~\ref{sec:profile}).
...
The theory of likelihood profiles is straightforward: the 
deviance (or likelihood) profile, $\devprof()$, 
for a focal model parameter $P$ is
the minimum value of the deviance conditioned on a particular value of
$P$.
For each parameter of interest, our goal is to evaluate the
deviance profile 
for many points --- optimizing over all of the non-focal parameters
each time --- over a wide enough range and with high enough resolution
to evaluate the shape of the profile
(in particular, whether it is quadratic, which would allow use of Wald
confidence intervals and tests) and to find the values of $P$ such
that $\devprof(P)=-2 {\mc L}(\widehat{P}) + \chi^2(1-\alpha)$, which represent the profile
confidence intervals.  While profile confidence intervals rely on
the asymptotic distribution of the minimum deviance, this is a much
weaker assumption than the log-quadratic likelihood surface
required by Wald tests.
%\bmb{more info/cites?}
...
above to profile LMMs. First, a parameterization of the random effects
variance-covariance matrix in terms of standard deviations and
correlations, or variances and covariances, is much more familiar to
users, and much more relevant as output of a statistical model, than
the parameters, $\bm\theta$, of the relative covariance factor ---
users are interested in inferences on variances or standard
deviations, not on $\bm\theta$. Second, the fixed-effect coefficients
and the residual standard deviation, both of which are also of
interest to users, are profiled out (in the sense used above) of the
deviance function (Section~\ref{sec:profdev}), so we have to find a
strategy for estimating the deviance for values of $\bm\beta$ and
$\sigma^2$ other than the profiled values.
...
correlations), and a value of the residual standard deviation, and
...
(Eq.~\ref{eq:residualVarianceEstimate}) in the deviance calculation.
...
In practice, the \code{profile} method \scwViaDb{systematically varies
  the parameters in a model, assessing the best possible fit that can
  be obtained with one parameter fixed at a specific value and
  comparing this fit to the globally optimal fit, which is the
  original model fit that allowed all the parameters to vary.  The
  models are compared according to the change in the deviance, which
  is the likelihood ratio test statistic.  We apply a signed square
  root transformation to this statistic and plot the resulting
  function, which we call the \emph{profile zeta function} or $\zeta$,
  versus the parameter value.} The signed aspect of this
transformation means that $\zeta$ is positive where the deviation from the parameter estimate is
positive and negative otherwise, leading to a monotonically increasing
function which is zero at the global optimum. \scwViaDb{ A $\zeta$
  value can be compared to the quantiles of the standard normal
  distribution.  For example, a 95\% profile deviance confidence
  interval on the parameter consists of the values for which $-1.96 <
  \zeta < 1.96$.  Because the process of profiling a fitted model
  can be computationally
  intensive --- it involves refitting the model many times ---
  one should exercise caution with complex models fit to
  large data sets.}
...
cutoff $\phi$
%\bmb{ugh, notation}
based on the $1-\alphamax$ critical
value of the $\chi^2$ distribution with a number of degrees of freedom
equal to the \emph{total} number of parameters in the model, where
$\alphamax$ is set to 0.05 by default. The profile computation
initially adjusts the focal parameter $p_i$ by an amount $\pstep=1.01
\hat p_i$ from its ML or REML estimate $\hat p_i$ (or by
$\pstep=0.001$ if $\hat p_i$ is zero, as in the case of a singular
variance-covariance model).  The local slope of the likelihood profile
$(\zeta(\hat p_i+\pstep)-\zeta(\hat p_i))/\pstep$ is used to pick the
next point to evaluate, extrapolating the local slope to find a new
$\pstep$ that would be expected to correspond to the desired step size
$\Delta \zeta$ (equal to $\phi/8$ by default, so that 16 points would
be used to cover the profile in both directions if the log-likelihood
surface were exactly quadratic).  Some fail-safe testing is done to
ensure that the step chosen is always positive, and less than a
maximum; if a deviance is ever detected that is lower than that of the
ML deviance, which can occur if the initial fit was wrong due to
numerical problems, the profiler issues an error and stops.
...
of \code{bootMer} are adapted from \code{merBoot}, a more ambitious
framework for bootstrapping \pkg{lme4} model fits which unfortunately
seems to be unavailable at present \citep{merBoot}.)

%% BMB: this description is actually only relevant to GLMMs --
%% for better or worse we do _not_ do this for LMMs, because
%% we would have to implement something via devfun2() with offsets
%% we might be able to prove that in this case beta and theta
%% are actually orthogonal ... ?
%%
%% While convenient and efficient because it makes use of
%% structures that have already been computed and stored as
%% part of the fitting process,
%% this computation is only accurate to the extent that
%% the fixed-effect ($\bm\beta$) and variance-covariance ($\thetavec$)
%% parameters are independent.  Recent versions of \code{lme4}
%% instead compute a finite-difference approximation of the Hessian
%% (of the full parameter vector $\{\thetavec,\bm\beta\}$).
...
Bayesian linear regression modelling \citep{gelman2013bayesian}
...
Eq.~\ref{eq:condVar} does not get computed directly, but rather an
...
\end{equation}
%% where $\Var(\mathbf{u|y}) = \mathbf{V}$ is the conditional covariance
%% of $\mathbf{u}$ derived in the preceding section
%% (\ref{sec:outputTheory}).
...
%% X <- getME(fm1, "X")
%% beta <- getME(fm1, "beta")
%% (V <- solve((Lambdat %*% Zt %*% Z %*% Lambda) + Diagonal(n = 36, rep(1, 36))))[1:2,1:2]  # ugly but i like the conceptual and high-level clarity
...
\begin{equation}
  ...
  \bm H = (\bm C_L\trans \bm C_L + \bm C_R\trans \bm C_R)\bm W
\end{equation}
where,
\begin{equation}
  \label{eq:CL}
  \bm L_{\bm\theta} \bm C_L = \bLt\trans \bm Z\trans
\end{equation}
and,
\begin{equation}
  \label{eq:CR}
  \bm R\trans_{X} \bm C_R = \bm X\trans - \bm R\trans_{ZX}\bm C_L
\end{equation}

The trace of the hat matrix is often used as a measure of the
effective degrees of freedom (e.g. \citet{vaida2005conditional}).
Using a relationship between the trace and vec operators, the trace of
$\bm H$ may be expressed as,
\begin{equation}
  \label{eq:traceHat}
  \tr(\bm H) = \left\| \VEC(\bm C_L \bm W^{1/2}) \right\|^2
  + \left\| \VEC(\bm C_R \bm W^{1/2}) \right\|^2
\end{equation}
...
which is the class of objects returned by
\code{lmer} (In addition to these methods, the \code{getME}
  function can be used to extract various objects from a fitted mixed
  model in \pkg{lme4}.) Here we illustrate the use of several of
these methods.
...
%    methods(class = 'merMod')[10] &  % family
    \code{fitted} &
    Fitted values given conditional modes (Eq.~\ref{eq:meanYgivenU}). \\
...
%    \code{fortify} &
%    Add information to data used to fit model. \\
%    methods(class = 'merMod')[15] \\
%   \code{isLMM} &
%   \code{TRUE} if the object is a linear mixed model. \\
%    methods(class = 'merMod')[17] \\ % isREML
%    methods(class = 'merMod')[18] \\
    \code{logLik} & % logLik
...
%    & Useful for computing confidence intervals. \\
%    methods(class = 'merMod')[28] \\
...
% ------------------------------------------------------------
% The following version of the methods table might be useful for
% sanity checks
% ------------------------------------------------------------
% <<methodsForTable, echo=FALSE>>=
% methodsForTable <- sub('.merMod', '', methods(class = 'merMod'))
% @
...
%     Generic (Section) & Brief description of return value \\
%     \hline
%     \Sexpr{methodsForTable[1]} (\ref{sec:summary})  &
%     Estimated variances, standard deviations, and correlations. \\
%     \Sexpr{methodsForTable[2]} (\ref{sec:modComp}) &
%     Decomposition of fixed-effects contributions \\
%     & or model comparison. \\
%     \Sexpr{methodsForTable[3]} &
%     Function returning  profiled deviance or REML criterion. \\
%     \Sexpr{methodsForTable[4]} &
%     Sum-of the random and fixed effects for each level. \\
%     \Sexpr{methodsForTable[5]} (\ref{sec:CI}) &
%     Confidence intervals on linear mixed-model parameters. \\
%     \Sexpr{methodsForTable[6]} (\ref{sec:summary}) &
%     Minus twice log-likelihood or REML criterion.\\
%     \Sexpr{methodsForTable[7]} &
%     Residual degrees of freedom. \\
%     \Sexpr{methodsForTable[8]} &
%     Drop allowable single terms from the model. \\
%     \Sexpr{methodsForTable[9]} &
%     Generalized Akaike's information criterion \\
% %    \Sexpr{methods(class = 'merMod')[10]} &  % family
%     \Sexpr{methodsForTable[11]} &
%     Fitted values given conditional modes (Eq.~\ref{eq:meanYgivenU}). \\
%     \Sexpr{methodsForTable[12]} (\ref{sec:summary}) &
%     Estimates of the fixed effects coefficients, $\widehat{\bm\beta}$ \\
%     \Sexpr{methodsForTable[13]} (\ref{sec:uncor}) &
%     Mixed-model formula of fitted model. \\
%     \Sexpr{methodsForTable[14]} &
%     Add information to data used to fit model. \\
% %    \Sexpr{methods(class = 'merMod')[15]} \\
%    \Sexpr{methodsForTable[16]} &
%    \code{TRUE} if the object is a linear mixed model. \\
% %    \Sexpr{methods(class = 'merMod')[17]} \\ % isREML
% %    \Sexpr{methods(class = 'merMod')[18]} \\
%     \Sexpr{methodsForTable[19]} & % logLik
%     Log-likelihood at the fitted values. \\
%     \Sexpr{methodsForTable[20]} & % model.frame
%     Data required to fit the model. \\
%     \Sexpr{methodsForTable[21]} &
%     Fixed effects model matrix, $\bm X$.\\
%     \Sexpr{methodsForTable[22]} (\ref{sec:summary}) &
%     Number of levels in each grouping factor. \\
%     \Sexpr{methodsForTable[23]} (\ref{sec:summary}) &
%     Number of observations.\\
%     \Sexpr{methodsForTable[24]} &
%     Diagnostic plots for mixed-model fits. \\
%     \Sexpr{methodsForTable[25]} &
%     Various types of predicted values. \\
%     \Sexpr{methodsForTable[26]} &
%     Basic printout of mixed-model objects. \\
%     \Sexpr{methodsForTable[27]} (\ref{sec:profile}) &
%     Profiled likelihood over various model parameters. \\
%     & Useful for computing confidence intervals. \\
% %    \Sexpr{methods(class = 'merMod')[28]} \\
%     \Sexpr{methodsForTable[29]} (\ref{sec:summary}) &
%     Conditional modes of the random effects. \\
%     \Sexpr{methodsForTable[30]} &
%     Mixed model fitted to new data. \\
%     \Sexpr{methodsForTable[31]} (\ref{sec:modComp}) &
%     A maximum likelihood fitted mixed model.  \\
%     \Sexpr{methodsForTable[32]}  (\ref{sec:summary}) &
%     Various types of residual values. \\
%     \Sexpr{methodsForTable[33]} (\ref{sec:summary}) &
%     Residual standard deviation. \\
%     \Sexpr{methodsForTable[34]} &
%     Simulated data from a fitted mixed model. \\
%     \Sexpr{methodsForTable[35]} (\ref{sec:summary}) &
%     Summary of a mixed model. \\
%     \Sexpr{methodsForTable[36]} &
%     Terms representation of a mixed model. \\
%     \Sexpr{methodsForTable[37]} (\ref{sec:update}) &
%     Update a mixed model with revised formula. \\
%     \Sexpr{methodsForTable[38]} (\ref{sec:summary}) &
%     Covariance matrix of the fixed effect estimates. \\
%     \Sexpr{methodsForTable[39]} &
%     Prior weights used in model fitting. \\
%     \hline
%   \end{tabular}
%   \caption{List of currently available  methods for objects of the class \code{merMod}.}
...
%% We will now consider a series of modifications to the \code{fm1} and
%% \code{fm2} models of the \code{sleepstudy} data fitted above in
%% Sections~\ref{sec:sleepstudy} and \ref{sec:uncor}.  These
%% modifications illustrate various ways to specify linear mixed models
%% with \pkg{lme4}.
...
indicate that the model was fitted by restricted maximum likelihood
(REML) as well as the value of the REML criterion
(Eq.~\ref{eq:REMLdeviance}) at convergence (which may also be
extracted using \code{deviance(fm1)}).  The beginning of the summary
also reproduces the model formula and the scaled Pearson residuals,
<<summary1,echo=FALSE>>=
ss <- summary(fm1)
...
@
The second piece of \code{summary} output provides information
...
@ 
The print method for \code{VarCorr} hides the internal structure of
...
the theory).  Finally, we have the correlations between the fixed
effect estimates
<<summary4, echo=FALSE>>=
...
influence measures, e.g. Cook's distance and
leverage plots), in a way modeled on the diagnostic graphics of
\pkg{nlme} \citep{R:Pinheiro+Bates:2000}.  In particular, the following
commands will show the standard fitted vs. residual, scale-location,
and quantile-quantile plots familiar from the
\code{plot} method in base \proglang{R}
for linear models (objects of class \code{lm}):
...
adapted from Bayesian methods, for exploring model fit. The user picks
...
discussed in more detail in Section~\ref{sec:pvalues}.

To understand how these quantities are computed, let $\bm R_i$ contain
the rows of $\bm R_X$ (Eq.~\ref{eq:blockCholeskyDecomp}) associated
...
\end{equation}
If $DF_i$ is the number of columns in $\bm R_i$, then the $F$
statistic for term $i$ is,
\begin{equation}
  \label{eq:Fstat}
...
\subsubsection{Computing $p$ values}
\label{sec:pvalues}
...
objects to the \code{anova} method (Section~\ref{sec:modComp}), as well as
alternatives to $p$~values such as confidence intervals.
...
As described above, \pkg{lme4} provides confidence intervals 
(using \code{confint}) via Wald
approximations (for fixed-effect parameters only), likelihood
profiling, or parametric bootstrapping (the \code{type} argument
selects the method).
%% \bmb{I was going to hide these; I figured that readers could understand
%%how to use \code{confint} without actually seeing the code, and
%%I didn't want to show the \code{oldNames} argument (now maybe less
%%important since I'm not trying to construct the figure).  I don't
%% think I get profile/bootstrap warnings on my system \ldots}
%% \scw{I've turned off nonconvergence warnings for profile and bootstrap
%%  CIs, but should they actually be displayed and discussed?}
...
## obsolete
## ,fig.cap="Comparison of confidence intervals \\bmb{not worth showing?? FIXME: get rid of x axis labels;  better display?}",fig.scap="CI comparison"
...
intervals in R, the intervals can be computed either directly from
...
In the running sleep study examples, the 95\% confidence intervals
...
graphics) for plotting the profile zeta functions and other related
quantities.
...
\item The \emph{profile density plot}
  ...
  distribution of the estimator of the parameter, which for
  mixed models can be very difficult, but it gives a
  reasonable approximation.  If the profile zeta plot is linear, then
  the profile density plot will be Gaussian.
\item The \emph{profile pairs plot}
  ...
  each parameter for fixed values of the other parameter.  While
  ...
  scale for the parameter.  The $\zeta$ scales provide, in some sense,
...
For users who want to build their own graphical displays of the profile,
there is an \code{as.data.frame} method that converts profile (\code{thpr})
objects to a more convenient format.
...
(Eq.~\ref{eq:distYgivenU}).  The \code{fitted} method retrieves fitted
...
\pkg{lme4} also provides a \code{simulate} method, which allows similar
flexibility in conditioning on random effects; in addition it allows
the user to choose (via the \code{use.u} argument) between
conditioning on the fitted conditional modes or choosing a new set
of simulated conditional modes (zero-mean Normal deviates chosen
according to the estimated random effects variance-covariance matrices).
Finally, the \code{simulate} method has a method for \code{formula}
objects, which allows for \emph{de novo} simulation in the absence
of a fitted model.  In this case, the user must specify the
random effects ($\bm\theta$), fixed effects ($\bm\beta$),
and residual standard deviation ($\sigma$) parameters
via the \code{newparams} argument.  The standard simulation
method (based on a \code{merMod} object) is the basis of parametric
bootstrapping (Section~\ref{sec:pb}) and posterior predictive
simulation (Section~\ref{sec:diagnostics}); \emph{de novo} simulation
based on a formula provides a flexible framework for power analysis.

\section{Conclusion}

Mixed models are an extremely useful but computationally intensive
approach. Computational limitations are especially
important because mixed models are commonly applied to moderately
large data sets ($10^4$--$10^6$ observations).
By developing an efficient, general, and (now) well-documented
platform for fitted mixed models in \proglang{R}, we hope
to provide both a practical tool for end users interested in
analyzing data and a reusable, modular framework for downstream
developers interested in extending the class of models that
...
We have learned much about linear mixed models in the process
of developing \pkg{lme4}, both from our own attempts to
develop robust and reliable procedures and from the
broad community of \pkg{lme4} users;
we have attempted to describe many of these lessons
here.  In moving forward, our main priorities are (1) to 
maintain the reference implementation of \pkg{lme4} on CRAN, 
developing relatively few new features; (2) to improve the 
flexibility, efficiency and scalability of 
mixed-model analysis across multiple compatible implementations,
including both the \pkg{MixedModels} package for \proglang{Julia}
and the experimental \code{flexLambda} branch of \pkg{lme4}.

\section*{Acknowledgements}

Rune Haubo Christensen, Henrik Singmann, and Bin Dai have
contributed code to the current version of \pkg{lme4}; the
...
for hosting a working group on \pkg{lme4}, and the other participants
in that group (Vincent Dorie, S{\o}ren H{\o}jsgaard, and Fabian Scheipl)
for intellectual contributions.
We would like to acknowledge an NSERC Discovery grant and
NSERC postdoctoral fellowship for funding.
\bmb{any other funding from D\&M?}
...
\section{Appendix: modularization examples}
\label{sec:modularExamples}

\subsection{Homogeneous covariance among random effects terms}
\label{sec:homoLmer}
...
with a random slope and intercept.  The \code{lmer}  function would
fit this formula to data by estimating two $2 \times 2$ covariance
matrices---one for each random effects term.  It is not possible to
use the current version of
\code{lmer} to fit a model in which both terms share the same
$2 \times 2$ covariance matrix.  We illustrate the use of the modular
functions in \pkg{lme4} to fit such a model.

We simulate data from this homogeneous covariance model using a
balanced design that crosses both grouping factors,
<<modularSimulation>>=
set.seed(1)
...
To fit \code{form} to \code{dat} using a homogeneous covariance model,
we construct the \code{homoLmer} function, which we describe in
sections.  To simplify the problem, we only allow the specification of
three arguments: \code{formula}, \code{data}, and \code{use.mkMerMod}.
This last argument specifies how to process the fitted model, which we
discuss below.  All other relevant parameters are set to \code{lmer}
defaults, unless otherwise noted below.  Here is the argument list for
\code{homoLmer},
<<homoLmerArguments, eval=FALSE>>=
homoLmer <- function(formula, data, use.mkMerMod = FALSE) {}
@
After saving the \code{call}, the next step is to parse the formula
and data using the \code{lFormula} modular function (setting
\code{REML} to \code{FALSE} for simplicity),
<<homoLmerMatchCallAndParseFormula, eval=FALSE>>=
    mc <- match.call()
    lfHetero <- lfHomo <- lFormula(formula = formula, data = data, REML = FALSE)
@
We then check to make sure that this \code{formula} is a candidate for
a homogeneous covariance model.  In particular, a homogeneous
covariance model only makes sense if each random effects term has the
same number of random effects coefficients,
...
@
We save the number of fixed effects coefficients, \code{p}, number of
covariance parameters per term, \code{nth}, and number random effects
terms, \code{n_trms},
<<homoLmerDimensions, eval=FALSE>>=
    ...
    n_trms <- length(cnms)
@ % $
We now modify the parsed random effects terms to have homogeneous
covariance structure, which requires three lines.  First, use only the
first \code{nth} elements of the covariance parameters, \code{theta},
as these will be repeated to generate common covariance structure
across random effects terms.  Second, match the lower bounds of the
covariance parameters to the new \code{theta}.  Third, adjust the
mapping from the new \code{theta} to the nonzero elements of the
transposed relative covariance factor, \code{Lambdat}.
...
@
Then we construct and minimize a deviance function for the homogeneous
model,
<<homoLmerFitModifiedModel, eval=FALSE>>=
    devf <- do.call(mkLmerDevfun, lfHomo)
    opt <- optimizeLmer(devf)
@
The object \code{opt} contains the optimized values of the homogeneous
covariance parameters.  However, in order to interpret and make
inferences about this model, we need to do more work.  We highlight
two general approaches to doing this, which in this example are
selected with the \code{use.mkMerMod} logical argument.  If
\code{use.mkMerMod} is set to \code{FALSE}, then the parsed data and
formula object, \code{lfHomo}, deviance function, \code{devf}, and
optimization results, \code{opt}, are returned,
<<homoLmerReturnRaw, eval=FALSE>>=
    if(!use.mkMerMod) {
        return(list(lf = lfHomo, devf = devf, opt = opt))
    }
@
This option means that the convenience functions of the \code{lme4}
output module (Section~\ref{sec:mkMerMod}) are not available because the resulting
object is not a \code{merMod} object.  However, constructing a
\code{merMod} object that will be treated appropriately by the
convenience functions of the output module is rather difficult, and
sometimes impossible, and should therefore always be done with
caution.  In particular, \code{mkMerMod} should be used to work with
specific functions from the output module; users should never
assume that because some aspects of the output module give appropriate
results with a \code{merMod} object constructed from a modular fit,
all aspects will work.  It will sometimes be
necessary, as we next illustrate, to extend the \code{merMod} class
and rewrite some methods from the output module that are specialized
for this new class.

Next we update the unmodified model by capturing the modified model
...
@ 
and installing the parameters in the environment of the unmodified
deviance function,
<<installDevfun, eval=FALSE>>=
    devf0(opt$par <- th)
@

Finally, we need to extend the \code{merMod} class,
<<extendMerMod>>=
...
\subsection{Homogeneous variance over all random effects}
\label{sec:homoVar}

We modify the \code{sleepstudy} example,
<<homoParse>>=
parsedFormula <- lFormula(formula = Reaction ~ Days + (Days|Subject),
                          data = sleepstudy)
@
such that the random slope and intercept are uncorrelated and have
identical variance
(because the slope and intercept have different units,
this example is not entirely sensible --- it is offered for
technical illustration only).
  The \code{reTrms} component of
\code{parsedFormula} contains the relevant information, and can be
modified using the \code{within} function,
<<homoWithin>>=
...
optimizerOutput <- optimizeLmer(devianceFunction)
@
Because \code{lme4} is not designed for post-processing of such
models, \code{mkMerMod} cannot be expected to give sensible results.
To illustrate this point, we use it anyway and point out where it
makes mistakes,
<<homoMkMerMod>>=
mkMerMod(   rho = environment(devianceFunction),
            opt = optimizerOutput,
         reTrms = parsedFormula$reTrms,
             fr = parsedFormula$fr)
@
Note that \code{mkMerMod} is attempting to estimate a correlation
between the slope and intercept, which makes no sense within the
current model.  When using modular functions, the user is responsible
for producing interpretable output.  For example, to find the residual
and random-effects standard deviation, we may use,
...
% \subsection{Correlations between observations on edges}
% \label{sec:edges}

% \scw{very rough now}

% <<dataSOExamples, echo=FALSE>>=
...
%                  0.732679, 0.478058, 0.325698)), .Names = c("node1", "node2",
%                   "X", "Y"), class = "data.frame", row.names = c(NA, -45L))
% @

% Consider a graph of $n$ nodes, such that every pair of nodes is
% connected by an edge.  Each of the $\binom{n}{2}$ edges is
% characterized by a pair of continuous variables, $x$ and $y$. Such a
% data set was posted on stack overflow (\scw{FIXME:  add link}),
% <<printDataSOExample>>=
% dat
% ggplot(dat) + geom_point(aes(X, Y, colour = node1))
% @
% The model of interest is one in which there is a linear relationship
% between $x$ and $y$ as a fixed slope and intercept, and in which edges
% that share a node are correlated in the residuals from the fixed
% effects relationship.  This is a difficult model to specify using the
% \code{lme4} formula module.  We therefore use the strategy of parsing
% an \code{lme4} formula for a model that is close to the desired model,
% and then modify the output before passing it to the remaining model
% fitting functions.  One such initial model contains a random intercept
% among one of the nodes,
% <<formulaSOExample>>=
% lf <- lFormula(Y ~ X + (1|node1), data=dat)
% @
% which results in the following transposed random effects model matrix,
% <<originalZtSOExample>>=
% image(lf$reTrms$Zt)
% @
% However, for some unstated reason, the OP wants to use the following model matrix,
% <<randomEffectsMatricesSOExample>>=
% Zl <- lapply(c("node1","node2"),
%          function(nm) fac2sparse(dat[[nm]], "d", drop=FALSE))
% Zt <- Zl[[1]] + Zl[[2]]
% image(Zt)
% @
% It is not easy to construct a mixed-model formula for generating this
% model matrix.  It is easier to update the transposed random effects
% model matrix in the \code{lf} object,
% <<updateZtSOExample>>=
% lf$reTrms$Zt <- Zt
% @
% and continue to fit the model,
% <<fitModelSoExample>>=
